...
In a previous lecture, we estimated the relationship between
...
More precisely, we need to make an assumption as to which _parametric class_ of distributions is generating the data.
...
Treisman [[Treisman, 2016](/courses/Introduction-to-Quantitative-Economics/References#id95)] is interested in estimating the number of billionaires in different countries.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/73.%20Maximum%20Likelihood%20Estimation_files/73.%20Maximum%20Likelihood%20Estimation_8_0.png)
</div>
...
Treisman’s main source of data is _Forbes’_ annual rankings of billionaires and their estimated net worth.

The dataset `mle/fp.dta` can be downloaded from [AER page](https://www.aeaweb.org/articles?id=10.1257/aer.p20161068).
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/73.%20Maximum%20Likelihood%20Estimation_files/73.%20Maximum%20Likelihood%20Estimation_12_0.png)
</div>
...
The standard formulation — the so-called _poisson regression_ model — is as follows:

$$
f(y_i \mid \mathbf{x}_i) = \frac{\mu_i^{y_i}}{y_i!} e^{-\mu_i}; \qquad y_i = 0, 1, 2, \ldots , \infty . \tag{73.1}
$$
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/73.%20Maximum%20Likelihood%20Estimation_files/73.%20Maximum%20Likelihood%20Estimation_15_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/73.%20Maximum%20Likelihood%20Estimation_files/73.%20Maximum%20Likelihood%20Estimation_18_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/73.%20Maximum%20Likelihood%20Estimation_files/73.%20Maximum%20Likelihood%20Estimation_21_0.png)
</div>
...
We can also ensure that this value is a _maximum_ (as opposed to a
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/73.%20Maximum%20Likelihood%20Estimation_files/73.%20Maximum%20Likelihood%20Estimation_31_0.png)
</div>
...
Now that we have a grasp of the underlying concepts, we can implement MLE in a practical application.

We will utilize the Poisson regression model in `statsmodels` to generate a more comprehensive output, including standard errors, test statistics, and additional details.

`statsmodels` employs the same algorithm as previously discussed to determine the maximum likelihood estimates.
...
```
    ...
    Date:                Wed, 04 Sep 2024   Pseudo R-squ.:                  0.2546
    Time:                        16:25:41   Log-Likelihood:                -3.3784
...
Now let's reproduce the findings from Daniel Treisman’s paper, [Russia’s Billionaires](https://www.aeaweb.org/articles?id=10.1257/aer.p20161068),
mentioned earlier in this lecture.

Treisman begins by estimating equation [(73.1)](#equation-poissonreg), where:

- $ y_i $ represents the $ \text{number of billionaires}\_i $
- $ x\_{i1} $ is $ \log{GDP\ per\ capita}\_i $
- $ x\_{i2} $ is $ \log{population}\_i $
- $ x\_{i3} $ is $ {years\ in\ GATT}\_i $ – the years of membership in GATT and WTO (to represent access to international markets)

The study focuses solely on the year 2008 for estimation.

We will prepare our variables for estimation as follows (you should have the
...
Next, we can apply the `Poisson` function from `statsmodels` to fit the
model.

We will employ robust standard errors as indicated in the author’s paper.
...
```
    ...
    Date:                Wed, 04 Sep 2024   Pseudo R-squ.:                  0.8574
    Time:                        16:25:41   Log-Likelihood:                -438.54
...
Success! The algorithm successfully achieved convergence in 9 iterations.

Our output indicates that GDP per capita, population size, and years of
membership in the General Agreement on Tariffs and Trade (GATT) are
positively associated with the number of billionaires in a country, as
anticipated.

Let’s also estimate the author’s more comprehensive models and present
...
The output suggests that the prevalence of billionaires is positively
linked to GDP per capita, population size, stock market
capitalization, and negatively linked to the top marginal income tax
rate.

To analyze our results by country, we can visualize the difference between
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/73.%20Maximum%20Likelihood%20Estimation_files/73.%20Maximum%20Likelihood%20Estimation_42_0.png)
</div>

As observed, Russia has significantly more billionaires than predicted by
the model (approximately 50 more than expected).

Treisman utilizes this empirical finding to explore potential reasons for
Russia’s surplus of billionaires, including the origins of wealth in
Russia, the political environment, and the history of privatization in
the years following the USSR.

## Summary

In this lecture, we employed Maximum Likelihood Estimation to estimate the
parameters of a Poisson model.

`statsmodels` offers additional built-in likelihood models such as
...
For greater flexibility, `statsmodels` allows for manual specification of the
distribution using the `GenericLikelihoodModel` class - an
example notebook is available
...
Suppose we aim to estimate the probability of an event $ y_i $
occurring, based on some observations.

We could apply a probit regression model, where the pmf of $ y_i $ is
...
$ \Phi $ denotes the _cumulative normal distribution_ and
constrains the predicted $ y_i $ to be within the range of 0 and 1 (as required
for a probability).

$ \boldsymbol{\beta} $ represents a vector of coefficients.

Following the example in the lecture, create a class to represent the
Probit model.

To start, determine the log-likelihood function and derive the gradient and
Hessian.

The `scipy` module `stats.norm` includes the functions necessary to
compute the cmf and pmf of the normal distribution.

## Solution to Exercise 73.1
...
Utilizing the **fundamental theorem of calculus**,... the derivative of a
...
Using these findings, we can construct a class for the Probit model as
follows.
...
Utilize the following dataset and initial values of $ \boldsymbol{\beta} $ to
estimate the MLE using the Newton-Raphson algorithm developed earlier in
the lecture
...
Note that the straightforward Newton-Raphson algorithm presented in this lecture
is highly sensitive to initial values, which may result in failure to
achieve convergence with varying starting values.

## Solution to Exercise 73.2
...
```
    ...
    Method:                           MLE   Df Model:                            2
    Date:                Wed, 04 Sep 2024   Pseudo R-squ.:                  0.2961
    Time:                        16:25:41   Log-Likelihood:                -2.3687
