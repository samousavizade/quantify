...
- [[Stokey _et al._, 1989](/courses/Introduction-to-Quantitative-Economics/References#id222)], chapter 2
- [[Ljungqvist and Sargent, 2018](/courses/Introduction-to-Quantitative-Economics/References#id186)], section 3.1
- [EDTC](http://johnstachurski.net/edtc.html), chapter 1
- [[Sundaram, 1996](/courses/Introduction-to-Quantitative-Economics/References#id224)], chapter 12
...
Consider an agent who owns an amount $ y*t \in \mathbb R*+ := [0, \infty) $ of a consumption good at time $ t $.

This output can either be consumed or invested.

When the good is invested, it is transformed one-for-one into capital.

The resulting capital stock, denoted here by $ k\_{t+1} $, will then be used for production.

Production is stochastic, in that it also depends on a shock $ \xi\_{t+1} $ realized at the end of the current period.
...
where $ f \colon \mathbb R*+ \to \mathbb R*+ $ is called the production function.

The resource constraint is

$$
k_{t+1} + c_t \leq y_t \tag{39.1}
$$
...
$$
\mathbb E \left[ \sum_{t = 0}^{\infty} \beta^t u(c_t) \right] \tag{39.2}
$$
...
In [(39.3)](#equation-og-conse) we are assuming that the resource constraint [(39.1)](#equation-outcsdp0) holds with equality — which is reasonable because $ u $ is strictly increasing and no output will be wasted at the optimum.

In summary, the agent’s aim is to select a path $ c_0, c_1, c_2, \ldots $ for consumption that is

1. nonnegative,
1. feasible in the sense of [(39.1)](#equation-outcsdp0),
1. optimal, in the sense that it maximizes [(39.2)](#equation-texs0-og2) relative to all other feasible consumption sequences, and
1. _adapted_, in the sense that the action $ c*t $ depends only on
   observable outcomes, not on future outcomes such as $ \xi*{t+1} $.

In the present context

- $ y*t $ is called the \_state* variable — it summarizes the “state of the world” at the start of each period.
- $ c*t $ is called the \_control* variable — a value chosen by the agent each period after observing the state.
...
This is quite intuitive, but if you wish you can find proofs in texts such as [[Stokey _et al._, 1989](/courses/Introduction-to-Quantitative-Economics/References#id222)] (section 4.1).

Hereafter we focus on finding the best Markov policy.

In our context, a Markov policy is a function $ \sigma \colon
\mathbb R*+ \to \mathbb R*+ $, with the understanding that states are mapped to actions via

$$
c_t = \sigma(y_t) \quad \text{for all } t
$$

In what follows, we will call $ \sigma $ a _feasible consumption policy_ if it satisfies

$$
0 \leq \sigma(y) \leq y
\quad \text{for all} \quad
y \in \mathbb R_+ \tag{39.4}
$$
...
$$
v_{\sigma}(y) =
\mathbb E \left[ \sum_{t = 0}^{\infty} \beta^t u(\sigma(y_t)) \right] \tag{39.7}
$$

when $ \{y_t\} $ is given by [(39.5)](#equation-firstp0-og2) with $ y_0 = y $.
...
$$
v^*(y) := \sup_{\sigma \in \Sigma} \; v_{\sigma}(y) \tag{39.8}
$$

The value function gives the maximal value that can be obtained from state $ y $, after considering all feasible policies.

A policy $ \sigma \in \Sigma $ is called **optimal** if it attains the supremum in [(39.8)](#equation-vfcsdp0) for all $ y \in \mathbb R\_+ $.

### The Bellman Equation

With our assumptions on utility and production functions, the value function as defined in [(39.8)](#equation-vfcsdp0) also satisfies a **Bellman equation**.
...
\right\}
\qquad (y \in \mathbb R_+) \tag{39.9}
$$

This is a _functional equation in_ $ v $.
...
> _The value function_ $ v^\* $ satisfies the Bellman equation

In other words, [(39.9)](#equation-fpb30) holds when $ v=v^\* $.
...
Given a continuous function $ v $ on $ \mathbb R\_+ $, we say that
...
provided after [(39.9)](#equation-fpb30).
...
u(c) + \beta \int v(f(y - c) z) \phi(dz)
\right\}
\qquad (y \in \mathbb R_+) \tag{39.11}
$$

In other words, $ T $ sends the function $ v $ into the new function
$ Tv $ defined by [(39.11)](#equation-fcbell20-optgrowth).

By construction, the set of solutions to the Bellman equation
[(39.9)](#equation-fpb30) _exactly coincides with_ the set of fixed points of $ T $.
...
continuous bounded functions on $ \mathbb R\_+ $ under the supremum distance
...
Consult, for example, section 12.2 of [EDTC](http://johnstachurski.net/edtc.html), [[Kamihigashi, 2012](/courses/Introduction-to-Quantitative-Economics/References#id181)] or [[Martins-da-Rocha and Vailakis, 2010](/courses/Introduction-to-Quantitative-Economics/References#id192)].
...
Later we will sacrifice some of this clarity and
...
described in earlier lectures the McCall model and
cake eating.
...
1. Build a function $ \hat v $ on the state space $ \mathbb R\_+ $ by
   linear interpolation, based on these data points.
1. Obtain and record the value $ T \hat v(y_i) $ on each grid point
   $ y_i $ by repeatedly solving [(39.11)](#equation-fcbell20-optgrowth).
1. Unless some stopping condition is satisfied, set
   $ \{ v_1, \ldots, v_I \} = \{ T \hat v(y_1), \ldots, T \hat v(y_I) \} $ and go to step 2.

### Scalar Maximization

To maximize the right hand side of the Bellman equation [(39.9)](#equation-fpb30), we are going to use
...
right hand side of the Bellman equation [(39.9)](#equation-fpb30).
...
In the last line, the expectation in [(39.11)](#equation-fcbell20-optgrowth) is
...
where $ \{\xi*i\}*{i=1}^n $ are IID draws from $ \phi $.
...
(For example, it preserves the contraction mapping property of the Bellman operator — see, e.g., [[Pál and Stachurski, 2013](/courses/Introduction-to-Quantitative-Economics/References#id131)].)
...
For this particular problem, an exact analytical solution is available (see [[Ljungqvist and Sargent, 2018](/courses/Introduction-to-Quantitative-Economics/References#id186)], section 3.1.2), with
...
\right] +
\frac{1}{1 - \alpha \beta} \ln y \tag{39.12}
$$
...
In theory, since $ \hat{v} $ is a fixed point, the resulting function should again be $ \hat{v} $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_files/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_29_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_files/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_31_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_files/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_38_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_files/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_41_0.png)
</div>
...
```
    ...
    CPU times: user 8.73 s, sys: 34 ms, total: 8.77 s
    Wall time: 8.53 s
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_files/39.%20Optimal%20Growth%20I%3A%20The%20Stochastic%20Optimal%20Growth%20Model_50_0.png)
</div>
...
```
    ...
    CPU times: user 1.12 s, sys: 6.15 ms, total: 1.13 s
    Wall time: 732 ms
