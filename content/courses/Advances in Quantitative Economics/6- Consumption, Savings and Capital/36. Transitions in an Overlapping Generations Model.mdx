...
[[Diamond, 1965](/courses/Introduction-to-Quantitative-Economics/References#id259)].

We’ll present the version that was analyzed in chapter 2 of Auerbach and
Kotlikoff (1987) [[Auerbach and Kotlikoff, 1987](/courses/Introduction-to-Quantitative-Economics/References#id260)].
...
> **Note**Auerbach and Kotlikoff use computer code to calculate transition paths for their models with long-lived people.
...
A government **policy** consists of five sequences $ \{G*t, D_t, \tau_t, \delta*{ot}, \delta*{yt}\}*{t=0}^\infty $ whose components are
...
- $ \delta\_{yt} $ – a lump sum tax on each young person at time $ t $
- $ \delta\_{ot} $ – a lump sum tax on each old person at time $ t $

An **allocation** is a collection of sequences $ \{C*{yt}, C*{ot}, K*{t+1}, L_t, Y_t, G_t\}*{t=0}^\infty $; constituents of the sequences include
...
- $ C\_{yt} $ – consumption of young person at time $ t \geq 0 $
- $ C\_{ot} $ – consumption of old person at time $ t \geq 0 $
- $ K\_{t+1} - K_t \equiv I_t $ – investment in physical capital at time $ t \geq 0 $
- $ G_t $ – government purchases

National income and product accounts consist of a sequence of equalities

- $ Y*t = C*{yt} + C*{ot} + (K*{t+1} - K_t) + G_t, \quad t \geq 0 $

A **price system** is a pair of sequences $ \{W*t, r_t\}*{t=0}^\infty $; constituents of a price sequence include rental rates for the factors of production
...
$$
Y_t  = K_t^\alpha L_t^{1-\alpha}, \quad \alpha \in (0,1) \tag{36.1}
$$
...
Young people at time $ t $ purchase government debt $ D\_{t+1} $ that matures at time $ t+1 $.

Government debt issued at $ t $ bears a before-tax net rate of interest rate of $ r\_{t} $ at time $ t+1 $.
...
$$
D_{t+1} = (1 + r_t)  D_t + G_t - T_t  . \tag{36.2}
$$
...
- rents capital to a representative firm for $ r\_{t} K_t $,
- pays taxes $ \tau_t r_t (K_t+ D_t) $ on its rental and interest earnings,
- pays a lump sum tax $ \delta\_{ot} $ to the government,
...
- pays a lump sum tax $ \delta\_{yt} $ to the goverment,
- spends $ C\_{yt} $ on consumption,
- acquires non-negative assets $ A*{t+1} $ consisting of a sum of physical capital $ K*{t+1} $ and one-period government bonds $ D\_{t+1} $ that mature at $ t+1 $.

> **Note**If a lump-sum tax is negative, it means that the government pays the person a subsidy.
...
We take output at time $ t $ as _numeraire_, so the price of output at time $ t $ is one.
...
$ \delta\_{ot} $ the government.

An old person’s budget constraint is

$$
C_{o0} = (1 + r_0 (1 - \tau_0)) A_0 - \delta_{ot} . \tag{36.4}
$$

An initial old person’s utility function is $ C\_{o0} $, so the person’s optimal consumption plan
is provided by equation [(36.4)](#equation-eq-hbudgetold).
...
A young person’s post-tax-and-transfer earnings are $ W*t (1 - \tau_t) - \delta*{yt} $.

At each $ t \geq 0 $, a young person chooses a consumption plan $ C*{yt}, C*{ot+1} $
to maximize the Cobb-Douglas utility function

$$
U_t  = C_{yt}^\beta C_{o,t+1}^{1-\beta}, \quad \beta \in (0,1) \tag{36.5}
$$
...
Solving the second equation of [(36.6)](#equation-eq-twobudgetc) for savings $ A\_{t+1} $ and substituting it into the first equation implies the present value budget constraint

$$
C_{yt} + \frac{C_{ot+1}}{1 + r_{t+1}(1 - \tau_{t+1})} = W_t (1 - \tau_t) - \delta_{yt} - \frac{\delta_{ot}}{1 + r_{t+1}(1 - \tau_{t+1})} \tag{36.7}
$$
...
where $ \lambda $ is a Lagrange multiplier on the intertemporal budget constraint [(36.7)](#equation-eq-onebudgetc).

After several lines of algebra, the intertemporal budget constraint [(36.7)](#equation-eq-onebudgetc) and the first-order conditions for maximizing $ {\mathcal L} $ with respect to $ C*{yt}, C*{ot+1} $
...
The first-order condition for minimizing Lagrangian [(36.8)](#equation-eq-lagc) with respect to the Lagrange multipler $ \lambda $ recovers the budget constraint [(36.7)](#equation-eq-onebudgetc),
which, using [(36.9)](#equation-eq-optconsplan) gives the optimal savings plan

$$
A_{t+1} = (1-\beta) [ (1- \tau_t) W_t - \delta_{yt}] + \beta \frac{\delta_{ot}}{1 + r_{t+1}(1 - \tau_{t+1})} \tag{36.10}
$$
...
Kotlikoff (1987) [[Auerbach and Kotlikoff, 1987](/courses/Introduction-to-Quantitative-Economics/References#id260)] began their analysis in chapter 2.
...
To get the special chapter 2 case of Auerbach and Kotlikoff (1987) [[Auerbach and Kotlikoff, 1987](/courses/Introduction-to-Quantitative-Economics/References#id260)], we set both $ \delta*{ot} $ and $ \delta*{yt} $ to zero.

As our special case of [(36.9)](#equation-eq-optconsplan), we compute the following consumption-savings plan for a representative young person:
...
Using [(36.3)](#equation-eq-firmfonc) and $ A_t = K_t + D_t $, we obtain the following closed form transition law for capital:

$$
K_{t+1}=K_{t}^{\alpha}\left(1-\tau_{t}\right)\left(1-\alpha\right)\left(1-\beta\right) - D_{t}\\ \tag{36.11}
$$

### Steady states

From [(36.11)](#equation-eq-klawclosed) and the government budget constraint [(36.2)](#equation-eq-govbudgetsequence), we compute **time-invariant** or **steady state values** $ \hat K, \hat D, \hat T $:
...
When we simulate transition paths, it is useful to distinguish **state variables** at time $ t $ such as $ K*t, Y_t, D_t, W_t, r_t $ from **control variables** that include $ C*{yt}, C*{ot}, \tau*{t}, G_t $.
...
1. from $ t=1 $, the government will keep $ D*t=\bar{D} $ and adjust $ \tau*{t} $ to collect taxation to pay for the government consumption and interest payments on the debt
...
We must define sequences of government expenditure $ \{G*t\}*{t=0}^{T} $ and debt level $ \{D*t\}*{t=0}^{T+1} $ in advance, then pass them to the solver.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_37_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_40_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_43_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_45_0.png)
</div>
...
> **Note**Although the consumptions in the new steady state are strictly higher, it is at a cost of fewer public services and goods.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_48_0.png)
</div>
...
In all these experiments, we maintained the assumption that lump sum taxes were absent so that $ \delta*{yt}=0, \delta*{ot}=0 $.
...
- sequences of optimal consumptions $ \{C*{yt}, C*{ot}\} $
- sequences of prices $ \{W_t, r_t\} $
- sequences of capital stock and output $ \{K_t, Y_t\} $
- sequences of tax rates, government assets (debt), government purchases $ \{\tau*t, D_t, G_t\, \delta*{yt}, \delta\_{ot}\} $
...
In our [Experiment 1: Tax cut](#exp-tax-cut) example, sequences $ \{D*t\}*{t=0}^{T} $ and $ \{G*t\}*{t=0}^{T} $ are exogenous.

In addition, we assume that the lump sum taxes $ \{\delta*{yt}, \delta*{ot}\}\_{t=0}^{T} $ are given and known to everybody inside the model.

We can solve for sequences of other equilibrium sequences following the steps below

1. guess prices $ \{W*t, r_t\}*{t=0}^{T} $ and tax rates $ \{\tau*t\}*{t=0}^{T} $
1. solve for optimal consumption and saving plans $ \{C*{yt}, C*{ot}\}\_{t=0}^{T} $, treating the guesses of future prices and taxes as true
1. solve for transition of the capital stock $ \{K*t\}*{t=0}^{T} $
...
Note that it requires knowing future prices $ r*{t+1} $ and tax rate $ \tau*{t+1} $.
...
Here is an example that computes optimal consumption $ C*y^\*=\hat{C}\_y $ in the steady state with $ \delta*{yt}=\delta\_{ot}=0, $ like one that we studied earlier
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_62_1.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_63_0.png)
</div>

Next, we activate lump sum taxes.

Let’s alter our [Experiment 1: Tax cut](#exp-tax-cut) fiscal policy experiment by assuming that the government also increases lump sum taxes for both young and old people $ \delta*{yt}=\delta*{ot}=0.005, t\geq0 $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_67_0.png)
</div>

Compared to [Experiment 1: Tax cut](#exp-tax-cut), the government raises lump-sum taxes to finance the increasing debt interest payment, which is less distortionary comparing to raising the capital income tax rate.
...
The government sets the lump sum taxes $ \delta*{y,t}=-\delta*{o,t}=10\% \hat{C}\_{y} $ starting from $ t=0 $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_files/36.%20Transitions%20in%20an%20Overlapping%20Generations%20Model_71_0.png)
</div>

An initial old person benefits, especially when the social security system is launched because they receive a transfer but pays nothing for it.

But in the long run, consumption rates of both young and old people decrease because the social security system decreases incentives to save.
