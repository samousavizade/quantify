...
This lecture and **Cass-Koopmans Competitive Equilibrium** describe a model that Tjalling Koopmans [[Koopmans, 1965](/courses/Introduction-to-Quantitative-Economics/References#id91)]
and David Cass[[, 1965](/courses/Introduction-to-Quantitative-Economics/References#id92)] used to analyze optimal growth.

The model extends the model of Robert Solow
described in an earlier lecture.
...
We describe two versions of the model, a planning problem in this lecture, and a competitive equilibrium in this lecture **Cass-Koopmans Competitive Equilibrium**.
...
To find $ c $, use feasibility constraint [(34.1)](#equation-eq-feas200) to conclude that

$$
c(\omega) = c = C.
$$

This line of argument indicates the special _aggregation theory_ that lies beneath outcomes in which a representative consumer
consumes amount $ C $.
...
maximize [(34.2)](#equation-utility-functional) subject to [(34.5)](#equation-allocation).
...
We now compute **first-order necessary conditions** for extremization of Lagrangian [(34.6)](#equation-eq-lagrangian201):
...
In computing [(34.10)](#equation-constraint2) we recognize that $ K_t $ appears
in both the time $ t $ and time $ t-1 $ feasibility constraints [(34.5)](#equation-allocation).

Restrictions [(34.12)](#equation-constraint4) come from differentiating with respect
to $ K\_{T+1} $ and applying the following **Karush-Kuhn-Tucker condition** (KKT)
...
Combining [(34.9)](#equation-constraint1) and [(34.10)](#equation-constraint2) gives
...
which for our utility function [(34.3)](#equation-utility-oneperiod) becomes the consumption **Euler
equation**
...
which we can combine with the feasibility constraint [(34.5)](#equation-allocation) to get
...
This is a pair of non-linear first-order difference equations that map $ C*t, K_t $ into $ C*{t+1}, K\_{t+1} $ and that an optimal sequence $ \vec C , \vec K $ must satisfy.

It must also satisfy the initial condition that $ K*0 $ is given and $ K*{T+1} = 0 $.
...
First-order necessary conditions
[(34.9)](#equation-constraint1), [(34.10)](#equation-constraint2), and
[(34.11)](#equation-constraint3) for the planning problem form a system of **difference equations** with
two boundary conditions:

- $ K_0 $ is a given **initial condition** for capital
- $ K*{T+1} =0 $ is a **terminal condition** for capital that we
  deduced from the first-order necessary condition for $ K*{T+1} $
  the KKT condition [(34.13)](#equation-kkt)
...
- Given $ \mu_0 $ and $ k_0 $, we could compute $ c_0 $ from
  equation [(34.9)](#equation-constraint1) and then $ k_1 $ from equation
  [(34.11)](#equation-constraint3) and $ \mu_1 $ from equation
  [(34.10)](#equation-constraint2).
- We could continue in this way to compute the remaining elements of
  $ \vec{C}, \vec{K}, \vec{\mu} $.

However, we woujld not be assured that the Kuhn-Tucker condition [(34.13)](#equation-kkt) would be satisfied.
...
- Compute $ K\_{T+1} $ and check whether it
  equals zero.
- If $ K\_{T+1} =0 $, we have solved the problem.
- If $ K\_{T+1} > 0 $, lower $ \mu_0 $ and try again.
- If $ K\_{T+1} < 0 $, raise $ \mu_0 $ and try again.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_18_0.png)
</div>

Evidently, our initial guess for $ \mu_0 $ is too high, so
initial consumption too low.

We know this because we miss our $ K\_{T+1}=0 $ target on the high
side.

Now we automate things with a search-for-a-good $ \mu*0 $
algorithm that stops when we hit the target $ K*{t+1} = 0 $.
...
If $ K\_{T+1}>0 $, we take it to be our new **lower** bound
on $ C_0 $.

If $ K\_{T+1}<0 $, we take it to be our new **upper** bound.
...
When $ K\_{T+1} $ gets close enough to $ 0 $ (i.e., within an error
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_23_0.png)
</div>
...
It is instructive to set $ K*0 $ equal
to the $ \lim*{T \rightarrow + \infty } K_t $, which we’ll call steady state capital.

In a steady state $ K\_{t+1} = K_t=\bar{K} $ for all very
large $ t $.

Evalauating feasibility constraint [(34.5)](#equation-allocation) at $ \bar K $ gives
...
all $ t $ into [(34.14)](#equation-l12) gives
...
For production function [(34.4)](#equation-production-function), this becomes
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_27_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_29_0.png)
</div>

Notice how the planner pushes capital toward the steady state, stays
near there for a while, then pushes $ K*t $ toward the terminal
value $ K*{T+1} =0 $ when $ t $ closely approaches $ T $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_31_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_33_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_37_0.png)
</div>
...
[(34.12)](#equation-constraint4) with
...
From [(34.17)](#equation-feasibility-constraint) the steady state saving rate equals
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_39_0.png)
</div>
...
We now describe a classic diagram that describes an optimal $ (K\_{t+1}, C_t) $ path.

The diagram has $ K $ on the ordinate axis and $ C $ on the coordinate axis.

Given an arbitrary and fixed $ K $, a fixed point $ C $ of the consumption Euler equation [(34.15)](#equation-eq-consn-euler)
satisfies
...
Next note that given a time-invariant arbitrary $ C $, a fixed point $ K $ of the feasibility condition [(34.5)](#equation-allocation) solves the following equation
...
A steady state $ \left(K_s, C_s\right) $ is a pair $ (K,C) $ that satisfies both equations [(34.18)](#equation-eq-tildec) and [(34.19)](#equation-eq-tildek).

It is thus the intersection of the two curves $ \tilde{C} $ and $ \tilde{K} $ that we’ll plot in Figure Fig. 34.1 below.
...
The following code generates Figure Fig. 34.1, which is patterned on a graph that appears on page 411 of [[Intriligator, 2002](/courses/Introduction-to-Quantitative-Economics/References#id3)].

Figure Fig. 34.1 is a classic “phase plane” with “state” variable $ K $ on the ordinate axis and “co-state” variable $ C $ on the coordinate axis.

Figure Fig. 34.1 plots three curves:

- the blue line graphs $ C = \tilde C (K) $ of fixed points described by equation [(34.18)](#equation-eq-tildec).
- the red line graphs $ K = \tilde K(C) $ of fixed points described by equation [(34.19)](#equation-eq-tildek)
...
- the arrows on the green line show the direction in which dynamics [(34.16)](#equation-eq-systemdynamics) push successive $ (K\_{t+1}, C_t) $ pairs.

In addition to the three curves, Figure Fig. 34.1 plots arrows that point where the dynamics [(34.16)](#equation-eq-systemdynamics) drive the system when, for a given $ K_0 $, $ C_0 $ is not on the stable manifold depicted in the green line.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_51_0.png)
</div>

## Concluding Remarks

In **Cass-Koopmans Competitive Equilibrium**, we study a decentralized version of an economy with exactly the same
...
In place of quantity choices made by the planner, there are market prices that are set by a _deus ex machina_ from outside the model, a so-called invisible hand.
...
studied in **Cass-Koopmans Competitive Equilibrium** is a foundational topic in general equilibrium theory and welfare economics.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/34.%20Cass-Koopmans%20Model_files/34.%20Cass-Koopmans%20Model_56_0.png)
</div>
