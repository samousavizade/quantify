...
So the optimal path $ c^_ := \{c^__t\}_{t=0}^\infty $ must satisfy
$ U'(c^\*) = 0 $.

> **Note**If you want to know exactly how the derivative $ U'(c^_) $ is
> defined, given that the argument $ c^_ $ is a vector of infinite
...
$ c^_\_t - h $
and increases it in the next period to $ c^_\_{t+1} + h $.
