---
title: Eliminating Cross Products
draft: false
summary: test
---
...
The problem is to choose $ \{x*{t+1}, u_t\}*{t=0}^\infty $ to maximize
...
is defined by a 4-tuple of matrices: $ (\hat{A}, B, \hat{R}, Q) $.
...
The matrices $ (\hat{A}, B, \hat{R}, Q) $ defining the equivalent problem and the value function, policy function matrices $ P, F^\* $ that solve it are related to the matrices $ (A, B, R, Q, H) $ defining the original problem and the value function, policy function matrices $ P, F $ that solve the original problem by
...
and $ w\_{t+1} $ is the time $ t+1 $ component of a sequence of i.i.d. $ p \times 1 $ normally distibuted
...
A consequence is that we can use the following algorithm to solve Kalman filtering problems that involve non zero covariances between state and signal noises.
...
The state reconstruction covariance matrix $ \Sigma $ for the original problem equals the state reconstruction covariance matrix for the transformed problem.
