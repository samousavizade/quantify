---
title: The Permanent Income Model
draft: false
summary: test
---
...
This lecture describes a rational expectations version of the famous permanent income model of Milton Friedman [[Friedman, 1956](/courses/Introduction-to-Quantitative-Economics/References#id164)].

Robert Hall cast Friedman’s model within a linear-quadratic setting [[Hall, 1978](/courses/Introduction-to-Quantitative-Economics/References#id165)].
...
- alternative formulations of the _state_ of a dynamic system
- the idea of _cointegration_
- impulse response functions
- the idea that changes in consumption are useful as predictors of movements in income

Background readings on the linear-quadratic-Gaussian permanent income model are Hall’s [[Hall, 1978](/courses/Introduction-to-Quantitative-Economics/References#id165)] and chapter 2 of [[Ljungqvist and Sargent, 2018](/courses/Introduction-to-Quantitative-Economics/References#id186)].
...
_information set_ $ \mathcal{F}\_t $.
...
One common class of martingales is the family of _random walks_.
...
for some IID zero mean _innovation_ sequence $ \{w_t\} $.
...
$$
\mathbb{E}_0 \left[ \sum_{t=0}^\infty \beta^t u(c_t) \right] \tag{59.1}
$$
...
The consumer maximizes [(59.1)](#equation-sprob1) by choosing a consumption, borrowing plan $ \{c*t, b*{t+1}\}\_{t=0}^\infty $ subject to the sequence of budget constraints

$$
c_t + b_t = \frac{1}{1 + r} b_{t+1} +  y_t   \quad t \geq 0 \tag{59.2}
$$
...
Finally, we impose the _no Ponzi scheme_ condition

$$
\mathbb{E}_0 \left[ \sum_{t=0}^\infty \beta^t b_t^2 \right] < \infty \tag{59.4}
$$
...
First-order conditions for maximizing [(59.1)](#equation-sprob1) subject to [(59.2)](#equation-sprob2) are

$$
\mathbb{E}_t [u'(c_{t+1})] = u'(c_t) , \qquad t = 0, 1, \ldots \tag{59.5}
$$

These optimality conditions are also known as _Euler equations_.

If you’re not sure where they come from, you can find a proof sketch in the
[appendix](#perm-income-appendix).

With our quadratic preference specification, [(59.5)](#equation-sprob4) has the striking implication that consumption follows a martingale:

$$
\mathbb{E}_t [c_{t+1}] = c_t \tag{59.6}
$$

(In fact, quadratic preferences are _necessary_ for this conclusion <sup><a href="#f2" id="f2-link">[1]</a></sup>.)

One way to interpret [(59.6)](#equation-sprob5) is that consumption will change only when
...
> **Note**One way to solve the consumer’s problem is to apply _dynamic programming_. We do this later. But first we use
...
1. the optimality condition [(59.6)](#equation-sprob5)
1. the period-by-period budget constraint [(59.2)](#equation-sprob2), and
1. the boundary condition [(59.4)](#equation-sprob3)

To accomplish this, observe first that [(59.4)](#equation-sprob3) implies $ \lim*{t \to \infty} \beta^{\frac{t}{2}} b*{t+1}= 0 $.

Using this restriction on the debt path and solving [(59.2)](#equation-sprob2) forward yields

$$
b_t = \sum_{j=0}^\infty \beta^j (y_{t+j} - c_{t+j}) \tag{59.7}
$$

Take conditional expectations on both sides of [(59.7)](#equation-sprob6) and use the martingale property of consumption and the _law of iterated expectations_ to deduce

$$
b_t = \sum_{j=0}^\infty \beta^j \mathbb{E}_t [y_{t+j}] - \frac{c_t}{1-\beta} \tag{59.8}
$$
...
= {r \over 1+r} \left[ \sum_{j=0}^\infty \beta^j \mathbb{E}_t [y_{t+j}] - b_t\right] \tag{59.9}
$$

where the last equality uses $ (1 + r) \beta = 1 $.

These last two equations assert that consumption equals _economic income_

- **financial wealth** equals $ -b_t $
- **non-financial wealth** equals $ \sum*{j=0}^\infty \beta^j \mathbb{E}\_t [y*{t+j}] $
...
The _state_ vector confronting the consumer at $ t $ is $ \begin{bmatrix} b_t & z_t \end{bmatrix} $.

Here

- $ z_t $ is an _exogenous_ component, unaffected by consumer behavior.
- $ b_t $ is an _endogenous_ component (since it depends on the decision rule).

Note that $ z_t $ contains all variables useful for forecasting the consumer’s future endowment.

It is plausible that current decisions $ c*t $ and $ b*{t+1} $ should
...
Combining this with [(59.9)](#equation-sprob8) gives
...
Using this equality to eliminate $ c_t $ in the budget constraint [(59.2)](#equation-sprob2) gives
...
A key is to use the fact that $ (1 + r) \beta = 1 $ and $ (I - \beta A)^{-1} = \sum\_{j=0}^{\infty} \beta^j A^j $.

We’ve now successfully written $ c*t $ and $ b*{t+1} $ as functions of $ b_t $ and $ z_t $.
...
Then we can express equation [(59.11)](#equation-pi-ssr) as

$$
\begin{aligned}
x_{t+1} & = \tilde A x_t + \tilde C w_{t+1} \\
\tilde y_t & = \tilde U x_t
\end{aligned} \tag{59.12}
$$
...
$$
\mu_{t+1} = \tilde A \mu_t
\quad \text{with} \quad \mu_0 \text{ given} \tag{59.13}
$$

$$
\Sigma_{t+1}  = \tilde A  \Sigma_t \tilde A' + \tilde C \tilde C'
\quad \text{with} \quad \Sigma_0 \text{ given} \tag{59.14}
$$
...
\end{aligned} \tag{59.15}
$$

#### A Simple Example with IID Income

To gain some preliminary intuition on the implications of [(59.11)](#equation-pi-ssr), let’s look at a highly stylized example where income is just IID.

(Later examples will investigate more realistic income streams.)

In particular, let $ \{w*t\}*{t = 1}^{\infty} $ be IID and scalar standard normal, and let
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/59.%20The%20Permanent%20Income%20Model_files/59.%20The%20Permanent%20Income%20Model_14_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/59.%20The%20Permanent%20Income%20Model_files/59.%20The%20Permanent%20Income%20Model_16_0.png)
</div>
...
Hall [[Hall, 1978](/courses/Introduction-to-Quantitative-Economics/References#id165)] suggested an insightful way to summarize the implications of LQ permanent income theory.

First, to represent the solution for $ b*t $, shift [(59.9)](#equation-sprob8) forward one period and eliminate $ b*{t+1} $ by using [(59.2)](#equation-sprob2) to obtain
...
If we add and subtract $ \beta^{-1} (1-\beta) \sum*{j=0}^\infty \beta^j \mathbb{E}\_t y*{t+j} $ from the right side of the preceding equation and rearrange, we obtain

$$
c_{t+1} - c_t = (1-\beta) \sum_{j=0}^\infty \beta^j
\left\{ \mathbb{E}_{t+1} [y_{t+j+1}] - \mathbb{E}_t [y_{t+j+1}] \right\} \tag{59.16}
$$

The right side is the time $ t+1 $ _innovation to the expected present value_ of the endowment process $ \{y_t\} $.

We can represent the optimal decision rule for $ (c*t, b*{t+1}) $ in the form of [(59.16)](#equation-sprob11) and [(59.8)](#equation-sprob7), which we repeat:

$$
b_t = \sum_{j=0}^\infty \beta^j \mathbb{E}_t [y_{t+j}] - {1 \over 1-\beta} c_t \tag{59.17}
$$

Equation [(59.17)](#equation-sprob7aa) asserts that the consumer’s debt due at $ t $ equals the expected present value of its endowment minus the expected present value of its consumption stream.
...
Using these formulas together with [(59.3)](#equation-sprob15ab) and substituting into [(59.16)](#equation-sprob11) and [(59.17)](#equation-sprob7aa) gives the following representation for the consumer’s optimum decision rule:
...
y_t & = U z_t \\
z_{t+1} & = A z_t + C w_{t+1}
\end{aligned} \tag{59.18}
$$

Representation [(59.18)](#equation-sprob16abcd) makes clear that
...
- Consumption is a random walk with innovation $ (1-\beta) U (I-\beta A)^{-1} C w\_{t+1} $.
- This is a more explicit representation of the martingale result in [(59.6)](#equation-sprob5).

### Cointegration

Representation [(59.18)](#equation-sprob16abcd) reveals that the joint process $ \{c_t, b_t\} $ possesses the property that Engle and Granger [[Engle and Granger, 1987](/courses/Introduction-to-Quantitative-Economics/References#id162)] called [cointegration](https://en.wikipedia.org/wiki/Cointegration).
...
To apply cointegration in the present context, suppose that $ z_t $ is asymptotically stationary <sup><a href="#fn-as" id="fn-as-link">[3]</a></sup>.

Despite this, both $ c_t $ and $ b_t $ will be non-stationary because they have unit roots (see [(59.11)](#equation-pi-ssr) for $ b_t $).

Nevertheless, there is a linear combination of $ c_t, b_t $ that _is_ asymptotically stationary.

In particular, from the second equality in [(59.18)](#equation-sprob16abcd) we have

$$
(1-\beta) b_t + c_t = (1 - \beta) U (I-\beta A)^{-1} z_t \tag{59.19}
$$
...
Equation [(59.19)](#equation-pi-spr) can be rearranged to take the form

$$
(1-\beta) b_t + c_t = (1-\beta) \mathbb{E}_t \sum_{j=0}^\infty \beta^j y_{t+j} \tag{59.20}
$$

Equation [(59.20)](#equation-sprob77) asserts that the _cointegrating residual_ on the left side equals the conditional expectation of the geometric sum of future incomes on the right <sup><a href="#f8" id="f8-link">[4]</a></sup>.

### Cross-Sectional Implications

Consider again [(59.18)](#equation-sprob16abcd), this time in light of our discussion of
distribution dynamics in the **lecture on linear systems**.

The dynamics of $ c_t $ are given by

$$
c_{t+1} = c_t + (1-\beta) U  (I-\beta A)^{-1} C w_{t+1} \tag{59.21}
$$
...
$$
\mathrm{Var}[c_t] = \mathrm{Var}[c_0] + t \, \hat \sigma^2 \tag{59.22}
$$
...
Equation [(59.22)](#equation-pi-vt) tells us that the variance of $ c_t $ increases over time at a rate proportional to $ t $.

A number of different studies have investigated this prediction and found some support for it
(see, e.g., [[Deaton and Paxson, 1994](/courses/Introduction-to-Quantitative-Economics/References#id158)], [[Storesletten _et al._, 2004](/courses/Introduction-to-Quantitative-Economics/References#id223)]).
...
In particular, the response of $ c*{t+j} $ to a unit increase in the innovation $ w*{t+1} $ is $ (1-\beta) U (I -\beta A)^{-1} C $ for all $ j \geq 1 $.
...
The endowment process defined by [(59.3)](#equation-sprob15ab) has the moving average representation

$$
y_{t+1} = d(L) w_{t+1} \tag{59.23}
$$

where

- $ d(L) = \sum\_{j=0}^\infty d_j L^j $ for some sequence $ d_j $, where $ L $ is the lag operator <sup><a href="#f4" id="f4-link">[5]</a></sup>
- at time $ t $, the consumer has an information set <sup><a href="#f5" id="f5-link">[6]</a></sup> $ w^t = [w_t, w_{t-1}, \ldots ] $
...
$$
\mathbb{E}_{t+1} [y_{t+j}] - \mathbb{E}_t [y_{t+j}] = d_{j-1} w_{t+1} \tag{59.24}
$$

Using [(59.24)](#equation-sprob120) in [(59.16)](#equation-sprob11) gives

$$
c_{t+1} - c_t = (1-\beta) d(\beta) w_{t+1} \tag{59.25}
$$
...
In both examples, the endowment follows the process $ y*t = z*{1t} + z\_{2t} $ where
...
- $ w\_{t+1} $ is an IID $ 2 \times 1 $ process distributed as $ N(0,I) $.
- $ z\_{1t} $ is a permanent component of $ y_t $.
- $ z\_{2t} $ is a purely transitory component of $ y_t $.

### Example 1

Assume as before that the consumer observes the state $ z_t $ at time $ t $.

In view of [(59.18)](#equation-sprob16abcd) we have

$$
c_{t+1} - c_t = \sigma_1 w_{1t+1} + (1-\beta) \sigma_2 w_{2t+1} \tag{59.26}
$$

Formula [(59.26)](#equation-consexample1) shows how an increment $ \sigma*1 w*{1t+1} $ to the permanent component of income $ z\_{1t+1} $ leads to

- a permanent one-for-one increase in consumption and
- no increase in savings $ -b\_{t+1} $

But the purely transitory component of income $ \sigma*2 w*{2t+1} $ leads to a permanent increment in consumption by a fraction $ 1-\beta $ of transitory income.

The remaining fraction $ \beta $ is saved, leading to a permanent increment in $ -b\_{t+1} $.

Application of the formula for debt in [(59.11)](#equation-pi-ssr) to this example shows that

$$
b_{t+1} - b_t = - z_{2t} = - \sigma_2 w_{2t} \tag{59.27}
$$

This confirms that none of $ \sigma*1 w*{1t} $ is saved, while all of $ \sigma*2 w*{2t} $ is saved.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/59.%20The%20Permanent%20Income%20Model_files/59.%20The%20Permanent%20Income%20Model_25_0.png)
</div>

Notice how the permanent income shock provokes no change in assets $ -b\_{t+1} $ and an immediate permanent change in consumption equal to the permanent increment in non-financial income.
...
Under this assumption, it is appropriate to use an _innovation representation_ to form $ A, C, U $ in [(59.18)](#equation-sprob16abcd).

The discussion in sections 2.9.1 and 2.11.3 of [[Ljungqvist and Sargent, 2018](/courses/Introduction-to-Quantitative-Economics/References#id186)] shows that the pertinent state space representation for $ y_t $ is
...
- $ a*t := y_t - E [ y_t \,|\, y*{t-1}, \ldots, y_0] $

In the same discussion in [[Ljungqvist and Sargent, 2018](/courses/Introduction-to-Quantitative-Economics/References#id186)] it is shown that $ K \in [0,1] $ and that $ K $ increases as $ \sigma_1/\sigma_2 $ does.
...
Applying formulas [(59.18)](#equation-sprob16abcd) implies

$$
c_{t+1} - c_t = [1-\beta(1-K) ] a_{t+1} \tag{59.28}
$$

where the endowment process can now be represented in terms of the univariate innovation to $ y_t $ as

$$
y_{t+1} - y_t = a_{t+1} - (1-K) a_t \tag{59.29}
$$

Equation [(59.29)](#equation-incomemaar) indicates that the consumer regards

- fraction $ K $ of an innovation $ a*{t+1} $ to $ y*{t+1} $ as _permanent_
- fraction $ 1-K $ as purely transitory

The consumer permanently increases his consumption by the full amount of his estimate of the permanent part of $ a*{t+1} $, but by only $ (1-\beta) $ times his estimate of the purely transitory part of $ a*{t+1} $.

Therefore, in total, he permanently increments his consumption by a fraction $ K + (1-\beta) (1-K) = 1 - \beta (1-K) $ of $ a\_{t+1} $.

He saves the remaining fraction $ \beta (1-K) $.

According to equation [(59.29)](#equation-incomemaar), the first difference of income is a first-order moving average.

Equation [(59.28)](#equation-consexample2) asserts that the first difference of consumption is IID.

Application of formula to this example shows that

$$
b_{t+1} - b_t = (K-1) a_t \tag{59.30}
$$
...
Further discussion can be found in, e.g., [[Hall and Mishkin, 1982](/courses/Introduction-to-Quantitative-Economics/References#id166)], [[Parker, 1999](/courses/Introduction-to-Quantitative-Economics/References#id201)], [[Deaton, 1991](/courses/Introduction-to-Quantitative-Economics/References#id157)], [[Carroll, 2001](/courses/Introduction-to-Quantitative-Economics/References#id153)].

## Appendix: The Euler Equation

Where does the first-order condition [(59.5)](#equation-sprob4) come from?
...
From the budget constraint [(59.2)](#equation-sprob2) we then have
...
Using $ \beta R = 1 $ gives [(59.5)](#equation-sprob4) in the two-period case.
