...
As we saw in our previous lecture on this topic, Robert Hall[[, 1978](/courses/Introduction-to-Quantitative-Economics/References#id165)] used the LQ permanent income model to restrict and interpret intertemporal comovements of nondurable consumption, nonfinancial income, and financial wealth.
...
For example, a model isomorphic to the LQ permanent income model has been used by Robert Barro [[Barro, 1979](/courses/Introduction-to-Quantitative-Economics/References#id149)] to interpret intertemporal comovements of a government’s tax collections, its expenditures net of debt service, and its public debt.
...
consumption dynamics in the spirit of Truman Bewley [[Bewley, 1986](/courses/Introduction-to-Quantitative-Economics/References#id113)].
...
$$
E_0 \sum_{t=0}^\infty \beta^t u(c_t) \tag{60.1}
$$

where $ u(c) = -(c - \gamma)^2 $.

The consumer maximizes [(60.1)](#equation-old1) by choosing a
consumption, borrowing plan $ \{c*t, b*{t+1}\}\_{t=0}^\infty $
subject to the sequence of budget constraints

$$
c_t + b_t = \frac{1}{1 + r} b_{t+1}  + y_t,
\quad t \geq 0 \tag{60.2}
$$

and the no-Ponzi condition

$$
E_0 \sum_{t=0}^\infty \beta^t b_t^2 < \infty \tag{60.3}
$$
...
Barro’s [[Barro, 1979](/courses/Introduction-to-Quantitative-Economics/References#id149)] model of tax smoothing.
...
We can map this into the linear state space framework in [(60.4)](#equation-sprob15ab2), as
discussed in our lecture on linear models.
...
Recall from our lecture on **LQ theory** that the optimal linear regulator problem is to choose
...
\qquad t\geq 0, \tag{60.5}
$$

where $ w\_{t+1} $ is IID with mean vector zero and $ \mathbb E w_t w'\_t= I $.

The tildes in $ \tilde A, \tilde B, \tilde C $ are to avoid clashing with notation in [(60.4)](#equation-sprob15ab2).
...
Under an optimal decision rule $ F $, the state vector $ x*t $ evolves according to $ x*{t+1} = (\tilde A-\tilde BF) x*t + \tilde C w*{t+1} $.
...
With this notation and $ U\_\gamma := \begin{bmatrix} \gamma & 0 & 0
\end{bmatrix} $, we can write the state dynamics as in [(60.5)](#equation-pilqsd) when
...
Please confirm for yourself that, with these definitions, the LQ dynamics [(60.5)](#equation-pilqsd) match the dynamics of $ z_t $ and $ b_t $ described above.
...
We have no direct way to capture the non-recursive restriction [(60.3)](#equation-old42)
...
In our first lecture on the infinite horizon permanent
...
In the spirit of Bewley models [[Bewley, 1986](/courses/Introduction-to-Quantitative-Economics/References#id113)], we’ll generate panels of consumers.
...
- The consumers are thus _ex-ante_ identical.
...
- Consumers are _ex-ante_ heterogeneous.
...
```
    ...
    /var/folders/zj/sz5g50055l9gj8nhh_6md1cw0000gn/T/ipykernel_38851/353008971.py:31: DeprecationWarning: Conversion of an array with ndim > 0 to a scalar is deprecated, and will error in future. Ensure you extract a single element from your array before performing this operation. (Deprecated NumPy 1.25.)
      cons_mean[t], cons_var[t] = μ_y[1], Σ_y[1, 1]
    /var/folders/zj/sz5g50055l9gj8nhh_6md1cw0000gn/T/ipykernel_38851/353008971.py:32: DeprecationWarning: Conversion of an array with ndim > 0 to a scalar is deprecated, and will error in future. Ensure you extract a single element from your array before performing this operation. (Deprecated NumPy 1.25.)
      debt_mean[t], debt_var[t] = μ_x[3], Σ_x[3, 3]
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_files/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_37_1.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_files/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_38_0.png)
</div>

Here is what is going on in the above graphs.

For our simulation, we have set initial conditions $ b*0 = y*{-1} = y\_{-2} = 0 $.

Because $ y*{-1} = y*{-2} = 0 $, nonfinancial income $ y*t $ starts far below its stationary mean $ \mu*{y, \infty} $ and rises early in each simulation.
...
$$
(1-\beta) b_t + c_t = (1-\beta) E_t \sum_{j=0}^\infty \beta^j y_{t+j} \tag{60.6}
$$
...
Thus, when we look at the panel of debt in the accompanying graph, we see that this is a group of _ex-ante_ identical people each of whom starts with zero debt.

All of them accumulate debt in anticipation of rising nonfinancial income.

They expect their nonfinancial income to rise toward the invariant distribution of income, a consequence of our having started them at $ y*{-1} = y*{-2} = 0 $.

#### Cointegration Residual

The following figure plots realizations of the left side of [(60.6)](#equation-old12), which,
as discussed in our last lecture, is called the **cointegrating residual**.

As mentioned above, the right side can be thought of as an
annuity payment on the expected present value of future income
$ E*t \sum*{j=0}^\infty \beta^j y\_{t+j} $.

Early along a realization, $ c*t $ is approximately constant while
$ (1-\beta) b_t $ and
$ (1-\beta) E_t \sum*{j=0}^\infty \beta^j y\_{t+j} $ both rise
...
_asymptotically_ covariance stationary, not _covariance stationary_.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_files/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_42_0.png)
</div>

### A “Borrowers and Lenders” Closed Economy

When we set $ y*{-1} = y*{-2} = 0 $ and $ b_0 =0 $ in the
...
We can use virtually the same code to rig a “poor man’s Bewley [[Bewley, 1986](/courses/Introduction-to-Quantitative-Economics/References#id113)] model” in the following way

- as before, we start everyone at $ b_0 = 0 $.
- But instead of starting everyone at $ y*{-1} = y*{-2} = 0 $, we
  draw $ \begin{bmatrix} y*{-1} \\ y*{-2} \end{bmatrix} $ from
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_files/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_44_1.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_files/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_45_0.png)
</div>
...
But now there is some initial dispersion because there is _ex-ante_ heterogeneity in the initial draws of $ \begin{bmatrix} y*{-1} \\ y*{-2} \end{bmatrix} $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_files/60.%20Permanent%20Income%20II%3A%20LQ%20Techniques_47_0.png)
</div>
