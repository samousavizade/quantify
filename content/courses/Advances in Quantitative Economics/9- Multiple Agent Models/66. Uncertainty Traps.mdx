...
In this lecture, we study a simplified version of an uncertainty traps model of Fajgelbaum, Schaal and Taschereau-Dumouchel [[Fajgelbaum _et al._, 2015](/courses/Introduction-to-Quantitative-Economics/References#id137)].
...
The original model described in [[Fajgelbaum _et al._, 2015](/courses/Introduction-to-Quantitative-Economics/References#id137)] has many interesting moving parts.
...
x_m = \theta + \epsilon_m
\quad \text{where} \quad
\epsilon_m \sim N \left(0, \gamma_x^{-1} \right) \tag{66.1}
$$
...
- $ X $ be the average output $ \frac{1}{M} \sum\_{m \in \mathbb M} x_m $ of the active firms.

With this notation and primes for next period values, we can write the updating of the mean and precision via

$$
\mu' = \rho \frac{\gamma \mu + M \gamma_x X}{\gamma + M \gamma_x} \tag{66.2}
$$
...
Exercise 1 provides more details on how [(66.2)](#equation-update-mean) and [(66.3)](#equation-update-prec) are derived and then asks you to fill in remaining steps.

The next figure plots the law of motion for the precision in [(66.3)](#equation-update-prec)
as a 45 degree diagram, with one curve for each $ M \in \{0, \ldots, 6\} $.

The other parameter values are $ \rho = 0.99, \gamma*x = 0.5, \sigma*\theta =0.5 $
...
$$
\mathbb E [ u(x_m - F_m) ] > c \tag{66.4}
$$

Here

- the mathematical expectation of $ x_m $ is based on [(66.1)](#equation-xgt) and beliefs $ N(\mu, \gamma^{-1}) $ for $ \theta $
...
The statement that $ F_m $ is pre-visible means that it is realized at the start of the period and treated as a constant in [(66.4)](#equation-pref1).

The utility function has the constant absolute risk aversion form

$$
u(x) = \frac{1}{a} \left(1 - \exp(-a x) \right) \tag{66.5}
$$

where $ a $ is a positive parameter.

Combining [(66.4)](#equation-pref1) and [(66.5)](#equation-pref2), entrepreneur $ m $ participates in the market (or is said to be active) when
...
The method to evaluate the number of active firms generates $ F*1,
\ldots, F*{\bar M} $ and tests condition [(66.6)](#equation-firm-test) for each firm.
...
Thus, the model gives us a _propagation mechanism_ that maps bad random draws into long downturns in economic activity.

## Exercises

## Exercise 66.1

Fill in the details behind [(66.2)](#equation-update-mean) and [(66.3)](#equation-update-prec) based on
the following standard result (see, e.g., p. 24 of [[Young and Smith, 2005](/courses/Introduction-to-Quantitative-Economics/References#id138)]).
...
then evaluate the distribution of $ \rho \theta + \sigma\_\theta w $
...
$ \gamma\_{t+1} $ against $ \gamma_t $ on a 45 degree diagram for
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/66.%20Uncertainty%20Traps_files/66.%20Uncertainty%20Traps_19_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/66.%20Uncertainty%20Traps_files/66.%20Uncertainty%20Traps_23_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/66.%20Uncertainty%20Traps_files/66.%20Uncertainty%20Traps_25_0.png)
</div>
