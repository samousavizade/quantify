...
the path of $ y^_\_t $ and therefore the paths of both components of
$ y_t = Q y^_\_t $ will diverge in absolute value as
$ t \rightarrow +\infty $. (We say that the paths _explode_)
...
Then from $ P*{t+1} = F^\* m*{t+1} $ and $ m*{t+1} = \rho m_t + \delta P_t $ we can deduce the recursion $ P*{t+1} = F^_ \rho m_t + F^_ \delta P_t $ and create the stacked system
...
1. For the system with $ \delta\neq 0 $ so that there is feedback,
   ...
   $ p_t = F^* m_t $ where $ F^* = Q*{21}Q\_{11}^{-1} $ as above.
1. Recalling the system [(64.11)](#equation-equation-3), [(64.12)](#equation-equation-4), and [(64.13)](#equation-equation-5) above, we define
   ...
   $ A = \begin{bmatrix}\rho & \delta \cr F^_ \rho & F^_\delta \end{bmatrix} $
...
1. We compute $ F_1 + F_2 F^_ $ and compare it
   with $ F^_ $ and check for the anticipated equality.
