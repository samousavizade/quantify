...
Other references include chapter 7 of [[Ljungqvist and Sargent, 2018](/courses/Introduction-to-Quantitative-Economics/References#id186)].
...
- Choice of price, output, location or capacity for firms in an industry (e.g., [[Ericson and Pakes, 1995](/courses/Introduction-to-Quantitative-Economics/References#id238)], [[Ryan, 2012](/courses/Introduction-to-Quantitative-Economics/References#id239)], [[Doraszelski and Satterthwaite, 2010](/courses/Introduction-to-Quantitative-Economics/References#id240)]).
- Rate of extraction from a shared natural resource, such as a fishery (e.g., [[Levhari and Mirman, 1980](/courses/Introduction-to-Quantitative-Economics/References#id241)], [[Van Long, 2011](/courses/Introduction-to-Quantitative-Economics/References#id243)]).
...
$$
p = a_0 - a_1 (q_1 +  q_2) \tag{65.1}
$$

Here $ p = p*t $ is the price of the good, $ q_i = q*{it} $ is the output of firm $ i=1,2 $ at time $ t $ and $ a_0 > 0, a_1 >0 $.

In [(65.1)](#equation-game2) and what follows,
...
$$
\pi_i = p q_i - \gamma (\hat q_i - q_i)^2, \quad \gamma > 0 , \tag{65.2}
$$

Substituting the inverse demand curve [(65.1)](#equation-game2) into [(65.2)](#equation-game1) lets us express the one-period payoff as

$$
\pi_i(q_i, q_{-i}, \hat q_i) = a_0 q_i - a_1 q_i^2 - a_1 q_i q_{-i} - \gamma (\hat q_i - q_i)^2 , \tag{65.3}
$$

where $ q\_{-i} $ denotes the output of the firm other than $ i $.

The objective of the firm is to maximize $ \sum*{t=0}^\infty \beta^t \pi*{it} $.

Firm $ i $ chooses a decision rule that sets next period quantity $ \hat q*i $ as a function $ f_i $ of the current state $ (q_i, q*{-i}) $.
...
Given $ f\_{-i} $, the Bellman equation of firm $ i $ is

$$
v_i(q_i, q_{-i}) = \max_{\hat q_i}
\left\{\pi_i (q_i, q_{-i}, \hat q_i) + \beta v_i(\hat q_i, f_{-i}(q_{-i}, q_i)) \right\} \tag{65.4}
$$

**Definition** A _Markov perfect equilibrium_ of the duopoly model is a pair of value functions $ (v_1, v_2) $ and a pair of policy functions $ (f_1, f_2) $ such that, for each $ i \in \{1, 2\} $ and each possible state,

- The value function $ v_i $ satisfies Bellman equation [(65.4)](#equation-game4).
- The maximizer on the right side of [(65.4)](#equation-game4) equals $ f*i(q_i, q*{-i}) $.
...
\left\{\pi_i (q_i, q_{-i}, \hat q_i) + \beta v^j_i(\hat q_i, f_{-i}(q_{-i}, q_i)) \right\} \tag{65.5}
$$
...
Player $ i $ takes $ \{u\_{-it}\} $ as given and minimizes
...
$$
x_{t+1} = A x_t + B_1 u_{1t} + B_2 u_{2t} \tag{65.7}
$$

Here

- $ x*t $ is an $ n \times 1 $ state vector and $ u*{it} $ is a $ k_i \times 1 $ vector of controls for player $ i $
- $ R_i $ is $ n \times n $
- $ S*i $ is $ k*{-i} \times k\_{-i} $
- $ Q_i $ is $ k_i \times k_i $
- $ W_i $ is $ n \times k_i $
- $ M*i $ is $ k*{-i} \times k_i $
- $ A $ is $ n \times n $
...
Player $ i $ employs linear decision rules $ u*{it} = - F*{it} x*t $, where $ F*{it} $ is a $ k_i \times n $ matrix.

A Markov perfect equilibrium is a pair of sequences $ \{F*{1t}, F*{2t}\} $ over $ t = t_0, \ldots, t_1 - 1 $ such that

- $ \{F*{1t}\} $ solves player 1’s problem, taking $ \{F*{2t}\} $ as given, and
- $ \{F*{2t}\} $ solves player 2’s problem, taking $ \{F*{1t}\} $ as given

If we take $ u*{2t} = - F*{2t} x_t $ and substitute it into [(65.6)](#equation-orig-1) and [(65.7)](#equation-orig-0), then player 1’s problem becomes minimization of
...
$$
x_{t+1} = \Lambda_{1t} x_t + B_1 u_{1t}, \tag{65.9}
$$

where

- $ \Lambda*{it} := A - B*{-i} F\_{-it} $
- $ \Pi*{it} := R_i + F*{-it}' S*i F*{-it} $
- $ \Gamma*{it} := W_i' - M_i' F*{-it} $
...
(\beta B_1' P_{1t+1} \Lambda_{1t} + \Gamma_{1t}) \tag{65.10}
$$

where $ P\_{1t} $ solves the matrix Riccati difference equation
...
\beta \Lambda_{1t}' P_{1t+1} \Lambda_{1t} \tag{65.11}
$$
...
(\beta B_2' P_{2t+1} \Lambda_{2t} + \Gamma_{2t}) \tag{65.12}
$$
...
\beta \Lambda_{2t}' P_{2t+1} \Lambda_{2t} \tag{65.13}
$$

Here, in all cases $ t = t*0, \ldots, t_1 - 1 $ and the terminal conditions are $ P*{it_1} = 0 $.

The solution procedure is to use equations [(65.10)](#equation-orig-3), [(65.11)](#equation-orig-4), [(65.12)](#equation-orig-5), and [(65.13)](#equation-orig-6), and “work backwards” from time $ t_1 - 1 $.

Since we’re working backward, $ P*{1t+1} $ and $ P*{2t+1} $ are taken as given at each stage.

Moreover, since

- some terms on the right-hand side of [(65.10)](#equation-orig-3) contain $ F\_{2t} $
- some terms on the right-hand side of [(65.12)](#equation-orig-5) contain $ F\_{1t} $

we need to solve these $ k_1 + k_2 $ equations simultaneously.

#### Key Insight

A key insight is that equations [(65.10)](#equation-orig-3) and [(65.12)](#equation-orig-5) are linear in $ F*{1t} $ and $ F*{2t} $.

After these equations have been solved, we can take $ F*{it} $ and solve for $ P*{it} $ in [(65.11)](#equation-orig-4) and [(65.13)](#equation-orig-6).

#### Infinite Horizon

We often want to compute the solutions of such games for infinite horizons, in the hope that the decision rules $ F\_{it} $ settle down to be time-invariant as $ t_1 \rightarrow +\infty $.
...
then we recover the one-period payoffs in expression [(65.3)](#equation-game3).

The law of motion for the state $ x*t $ is $ x*{t+1} = A x*t + B_1 u*{1t} + B*2 u*{2t} $ where
...
The optimal decision rule of firm $ i $ will take the form $ u\_{it} = - F_i x_t $, inducing the following closed-loop system for the evolution of $ x $ in the Markov perfect equilibrium:

$$
x_{t+1} = (A - B_1 F_1 -B_1 F_2 ) x_t \tag{65.14}
$$
...
In particular, let’s take F2 as computed above, plug it into [(65.8)](#equation-eq-mpe-p1p) and [(65.9)](#equation-eq-mpe-p1d) to get firm 1’s problem and solve it using LQ.
...
Given our optimal policies $ F1 $ and $ F2 $, the state evolves according to [(65.14)](#equation-eq-mpe-cle).

The following program

- imports $ F1 $ and $ F2 $ from the previous program along with all parameters.
- computes the evolution of $ x_t $ using [(65.14)](#equation-eq-mpe-cle).
- extracts and plots industry output $ q*t = q*{1t} + q\_{2t} $ and price $ p_t = a_0 - a_1 q_t $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/65.%20Markov%20Perfect%20Equilibrium_files/65.%20Markov%20Perfect%20Equilibrium_25_0.png)
</div>

Note that the initial condition has been set to $ q*{10} = q*{20} = 1.0 $.
...
The monopolist initial condition is $ q*0 = 2.0 $ to mimic the industry initial condition $ q*{10} = q\_{20} = 1.0 $ in the MPE case.
...
in the law of motion $ x\_{t+1} = A x_t + B u_t $.
...
```
    ...
    /var/folders/zj/sz5g50055l9gj8nhh_6md1cw0000gn/T/ipykernel_39199/1048199155.py:13: DeprecationWarning: Conversion of an array with ndim > 0 to a scalar is deprecated, and will error in future. Ensure you extract a single element from your array before performing this operation. (Deprecated NumPy 1.25.)
      qm[i] = float(x) + q_bar
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/65.%20Markov%20Perfect%20Equilibrium_files/65.%20Markov%20Perfect%20Equilibrium_36_0.png)
</div>
...
It takes the form of infinite horizon linear-quadratic game proposed by Judd [[Judd, 1990](/courses/Introduction-to-Quantitative-Economics/References#id180)].
...
- $ I\_{it} $ = inventories of firm $ i $ at beginning of $ t $
- $ q\_{it} $ = production of firm $ i $ during period $ t $
- $ p\_{it} $ = price charged by firm $ i $ during period $ t $
- $ S\_{it} $ = sales made by firm $ i $ during period $ t $
- $ E\_{it} $ = costs of production of firm $ i $ during period $ t $
- $ C\_{it} $ = costs of carrying inventories for firm $ i $ during $ t $

The firms’ cost functions are

- $ C*{it} = c*{i1} + c*{i2} I*{it} + 0.5 c*{i3} I*{it}^2 $
- $ E*{it} = e*{i1} + e*{i2}q*{it} + 0.5 e*{i3} q*{it}^2 $ where $ e*{ij}, c*{ij} $ are positive scalars
...
- $ S*t = \begin{bmatrix} S*{1t} & S\_{2t} \end{bmatrix}' $
...
Decision rules for price and quantity take the form $ u\_{it} = -F_i x_t $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/65.%20Markov%20Perfect%20Equilibrium_files/65.%20Markov%20Perfect%20Equilibrium_47_0.png)
</div>
