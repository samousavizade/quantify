...
Given a set of vectors $ A := \{a_1, \ldots, a_k\} $ in $ \mathbb R ^n $, we can create new vectors through linear operations. These new vectors are called _linear combinations_ of $ A $.
...
The values $ \beta_1, \ldots, \beta_k $ are the _coefficients_ of the linear combination. The set of all linear combinations of $ A $ is called the _span_ of $ A $.
...
If $ A = \{e_1, e_2, e_3\} $ consists of the _canonical basis vectors_ of $ \mathbb R ^3 $:
...
For a square matrix, the elements $ a\_{ii} $ form the _principal diagonal_.
