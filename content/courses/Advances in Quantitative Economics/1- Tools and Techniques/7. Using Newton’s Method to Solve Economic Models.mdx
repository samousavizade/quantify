...
In other words, we seek a $ k^_ > 0 $ such that $ g(k^_)=k^\* $.

- such a $ k^_ $ is termed a [steady state](https://en.wikipedia.org/wiki/Steady_state),
  since $ k_t = k^_ $ implies $ k\_{t+1} = k^\* $.
...
An equilibrium price vector $ p^_ $ satisfies $ e_i(p^_) = 0 $.
...
If these two contour lines intersect at some price vector $ p^_ $, then $ p^_ $ is an equilibrium price vector.
...
- The calculation of the fixed point is equivalent to finding $ k^_ $ such that $ f(k^_) - k^\* = 0 $.
