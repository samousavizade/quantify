...
In **A Problem that Stumped Milton Friedman**, we discussed a dilemma
that a Navy Captain presented to Milton Friedman during World War II.

The Navy instructed the Captain to apply a decision rule for quality control that the Captain believed
could be surpassed by a superior rule.

(The Navy had mandated the Captain to implement a specific **frequentist decision rule**.)

Milton Friedman recognized the Captain’s hypothesis as presenting a complex statistical challenge that he and other
members of the US Government’s Statistical Research Group at Columbia University sought to address.

One member of the group, the renowned mathematician Abraham Wald, quickly resolved the issue.

A productive approach to formulate the problem is to utilize concepts from Bayesian statistics that we outline in
this lecture **Exchangeability and Bayesian Updating** and in this lecture
**Likelihood Ratio Processes**, which illustrates the connection between Bayesian
updating and likelihood ratio processes.

This lecture employs Python to create simulations that assess expected losses under **frequentist** and **Bayesian**
decision rules for a version of the Navy Captain’s decision dilemma.

The simulations confirm the Navy Captain’s intuition that a more effective rule exists than the one mandated by the Navy.

## Setup

To clarify the dilemma of the Navy Captain whose inquiries posed the
problem that Milton Friedman and Allan Wallis presented to Abraham
Wald, we analyze a scenario with the following components.

- Each period, a decision maker draws a non-negative random variable
  $ Z $ from a probability distribution that he does not fully
  comprehend. He is aware that two probability distributions are possible,
  $ f*{0} $ and $ f*{1} $, and that whichever distribution it
is remains constant over time. The decision maker believes that prior to
the beginning of time, nature definitively selected either
$ f*{0} $ or $ f_1 $ and that the likelihood it
  selected $ f*{0} $ is probability $ \pi^{\*} $.
- The decision maker observes a sample
  $ \left\{ z*{i}\right\} *{i=0}^{t} $ from the distribution
  chosen by nature.

The decision maker aims to determine which distribution actually governs
$ Z $ and is concerned by two types of errors and the associated losses.

- a loss $ \bar L*{1} $ from a **type I error** that occurs when he concludes that
  $ f=f*{1} $ when in fact $ f=f\_{0} $
- a loss $ \bar L*{0} $ from a **type II error** that occurs when he concludes that
  $ f=f*{0} $ when in fact $ f=f\_{1} $

The decision maker incurs a cost $ c $ for drawing
another $ z $

We primarily adopt parameters from the quantecon lecture
**A Problem that Stumped Milton Friedman** except that we increase both $ \bar L*{0} $
and $ \bar L*{1} $ from $ 25 $ to $ 100 $ to motivate the
frequentist Navy Captain to take more draws before making a decision.

We set the cost $ c $ of taking one additional draw at $ 1.25 $.

We define the probability distributions $ f*{0} $ and $ f*{1} $ as
beta distributions with $ a*{0}=b*{0}=1 $, $ a_{1}=3 $, and
$ b\_{1}=1.2 $, respectively.

Below is some Python code that establishes these objects.
...
We commence by defining a `jitclass` that retains parameters and
functions necessary to address issues for both the Bayesian and
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_10_0.png)
</div>

Above, we illustrate the two potential probability densities $ f_0 $ and
$ f_1 $
...
Let
$ L\left(z^{t}\right)=\prod*{i=0}^{t}\frac{f*{0}\left(z*{i}\right)}{f*{1}\left(z*{i}\right)} $
be the likelihood ratio associated with observing the sequence
$ \left\{ z*{i}\right\} \_{i=0}^{t} $.
...
lecture **Likelihood Ratio Processes**.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_15_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_20_0.png)
</div>

Our frequentist minimizes the expected total loss presented in equation
[(55.1)](#equation-val1) by choosing $ \left(t,d\right) $.
...
$ \pi^{\*}=\Pr\left\{ \text{nature selects }f\_{0}\right\} =0.5 $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_25_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_30_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_32_0.png)
</div>

## Bayesian Decision Rule

In **A Problem that Stumped Milton Friedman**,
...
lecture **A Problem that Stumped Milton Friedman**
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_37_0.png)
</div>
...
conditioning on knowing for sure that nature has selected $ f*{0} $,
in the first case, or $ f*{1} $, in the second case.
...
where
$ \pi^{\prime}=\frac{\pi f*{0}\left(z^{\prime}\right)}{\pi f*{0}\left(z^{\prime}\right)+\left(1-\pi\right)f\_{1}\left(z^{\prime}\right)} $.

Given a prior probability $ \pi\_{0} $, the expected loss for the
Bayesian is
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_40_0.png)
</div>

Given an assumed value for
$ \pi^{_}=\Pr\left\{ \text{nature selects }f_{0}\right\} $, we can
then compute $ \bar{V}\*{Bayes}\left(\pi\_{0}\right) $.

We can then determine an initial Bayesian prior $ \pi\_{0}^{\*} $ that
minimizes this objective concept of expected loss.

The figure 9 below plots four cases corresponding to
$ \pi^{\*}=0.25,0.3,0.5,0.7 $.

We observe that in each case $ \hat{\pi\_{0}} $ equals $ \hat{\pi} $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_43_0.png)
</div>

This pattern of outcomes holds more generally.

Thus, the following Python code generates the associated graph that
verifies the equality of $ \hat{\pi\_{0}} $ to $ \hat{\pi} $ holds
for all $ \pi^{\*} $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_45_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_49_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_51_0.png)
</div>
...
conditional on distribution $ q=f*{0} $ or $ q= f*{1} $
generating the data.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_62_0.png)
</div>
...
the one-to-one mapping from $ L*{t} $ to $ \pi*{t} $ given
$ \pi_0 $ described in this lecture **Likelihood Ratio Processes**.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_65_0.png)
</div>

The above figures compare averages and variances of updated Bayesian
posteriors after $ t $ draws.

The left graph compares $ E\left(\pi*{t}\right) $ under
$ f*{0} $ to $ 1-E\left(\pi*{t}\right) $ under $ f*{1} $:
...
maker waits longer to decide when $ f\_{1} $ generates the data.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_67_0.png)
</div>
...
under $ f*{1} $ is the optimal probability of detection given
$ t $ that we defined earlier, and similarly it equals $ 1 $
minus the optimal probability of a false alarm under $ f*{0} $.
...
$ t $ _and_ that the decision is correct.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_70_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_72_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_75_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_files/55.%20Bayesian%20versus%20Frequentist%20Decision%20Rules_77_0.png)
</div>
