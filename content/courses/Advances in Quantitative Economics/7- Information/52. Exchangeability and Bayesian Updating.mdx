...
# Exchangeability and Bayesian Updating: An Overview

## Introduction

This lecture explores learning through Bayes’ Law.

We delve into the foundations of Bayesian statistical inference, as introduced by Bruno DeFinetti [[de Finetti, 1937](/courses/Introduction-to-Quantitative-Economics/References#id46)].

The significance of DeFinetti’s work for economists is highlighted in chapter 11 of [[Kreps, 1988](/courses/Introduction-to-Quantitative-Economics/References#id110)] by David Kreps.

An example we examine in this lecture is crucial to this lecture that extends the classic job search model of McCall [[McCall, 1970](/courses/Introduction-to-Quantitative-Economics/References#id194)] by incorporating a statistical inference problem for an unemployed worker.

Here, we create graphs to illustrate the role of the likelihood ratio in Bayes’ Law.

These graphs provide insights into the mechanics driving outcomes in this lecture about learning in an augmented McCall job search model.
...
- exchangeable (also known as _conditionally_ independently and identically distributed)
...
Because another term for **exchangeable** is **conditionally independent**, we want to convey an answer to the question _conditional on what?_

We also explain why an assumption of independence precludes learning while an assumption of conditional independence makes learning possible.
...
We begin by examining the concept of an **independently and identically distributed sequence** of random variables.

An independently and identically distributed sequence is often abbreviated as IID.

Two key notions are involved:
...
The sequence $ W_0, W_1, \ldots $ is **independently and identically distributed** (IID) if, in addition, the marginal density of $ W_t $ is the same for all $ t =0, 1, \ldots $.
...
### IID Means Past Observations Don’t Provide Information About Future Observations

If a sequence of random variables is IID, past information does not provide any information about future realizations.

Therefore, there is **nothing to learn** from the past about the future.

To understand these statements, let the joint distribution of a sequence of random variables $ \{W*t\}*{t=0}^T $
...
and partial history $ W\_{t-1}, \ldots, W_0 $ contains no information about the probability of $ W_t $.
...
## A Scenario Where Past Observations Are Informative

Let $ \{W*t\}*{t=0}^\infty $ be a sequence of nonnegative scalar random variables with a joint probability distribution constructed as follows.
...
We could say that _objectively_, meaning _after_ nature has chosen either $ F $ or $ G $, the probability that the data are generated as draws from $ F $ is either $ 0 $ or $ 1 $.

We now introduce a partially informed decision maker who knows both $ F $ and $ G $ but not which distribution nature selected at $ t = -1 $.
...
Thus, we assume that the decision maker knows both $ F $ and $ G $ but does not know which distribution nature has drawn. He expresses his ignorance by acting as if nature chose distribution $ F $ with probability $ \tilde \pi \in (0,1) $ and distribution $ G $ with probability $ 1 - \tilde \pi $.

At date $ t \geq 0 $, the decision maker knows the partial history $ w*t, w*{t-1}, \ldots, w_0 $.
...
Thus, **conditional on nature having selected** $ F $, the sequence $ W_0, W_1, \ldots $ is independently and identically distributed.
...
$$
h(W_0, W_1, \ldots ) \equiv \tilde \pi [f(W_0) f(W_1) \cdots \ ] + ( 1- \tilde \pi) [g(W_0) g(W_1) \cdots \ ] \tag{52.1}
$$
...
While the sequence $ W_0, W_1, \ldots $ is not IID, it can be verified that it is **exchangeable**, which means that the ``re-ordered’’ joint distributions $ h(W_0, W_1) $ and $ h(W_1, W_0) $ satisfy
...
Equation [(52.1)](#equation-eq-definetti) represents our instance of an exchangeable joint density over a sequence of random variables as a **mixture** of two IID joint densities over a sequence of random variables.
...
But how can we learn? And about what? The answer to the _about what_ question is $ \tilde \pi $.

The answer to the _how_ question is to use Bayes’ Law. Another way to say _use Bayes’ Law_ is to say _from a (subjective) joint distribution, compute an appropriate conditional distribution_.
...
The distribution of $ w\_{t+1} $ conditional on $ w^t $ is then

$$
\pi_t f + (1 - \pi_t) g .
$$

Bayes’ rule for updating $ \pi\_{t+1} $ is

$$
\pi_{t+1} = \frac{\pi_t f(w_{t+1})}{\pi_t f(w_{t+1}) + (1 - \pi_t) g(w_{t+1})} \tag{52.2}
$$

Equation [(52.2)](#equation-eq-bayes102) follows from Bayes’ rule, which tells us that
...
Let’s analyze and rearrange Bayes’ Law as represented in equation [(52.2)](#equation-eq-bayes102) to understand how the **posterior** probability $ \pi\_{t+1} $ is influenced by the **prior** probability $ \pi_t $ and the **likelihood ratio**

$$
l(w) = \frac{f(w)}{g(w)}
$$

It is helpful to rewrite the updating rule [(52.2)](#equation-eq-bayes102) as
...
Notice how the likelihood ratio and the prior interact to determine whether an observation $ w\_{t+1} $ leads the decision maker to increase or decrease the subjective probability he/she attaches to distribution $ F $.

When the likelihood ratio $ l(w*{t+1}) $ exceeds one, the observation $ w*{t+1} $ nudges the probability $ \pi $ upward, and when the likelihood ratio $ l(w*{t+1}) $ is less than one, the observation $ w*{t+1} $ nudges $ \pi $ downward.

Representation [(52.3)](#equation-eq-bayes103) is the foundation of some graphs that we’ll use to display the dynamics of $ \{\pi*t\}*{t=0}^\infty $ that are induced by Bayes’ Law.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/52.%20Exchangeability%20and%20Bayesian%20Updating_files/52.%20Exchangeability%20and%20Bayesian%20Updating_14_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/52.%20Exchangeability%20and%20Bayesian%20Updating_files/52.%20Exchangeability%20and%20Bayesian%20Updating_16_0.png)
</div>
...
We begin by generating $ N $ simulated $ \{\pi*t\} $ paths with $ T $ periods when the sequence is truly IID draws from $ F $. We set an initial prior $ \pi*{-1} = .5 $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/52.%20Exchangeability%20and%20Bayesian%20Updating_files/52.%20Exchangeability%20and%20Bayesian%20Updating_24_0.png)
</div>
...
Next, we generate paths with $ T $ periods when the sequence is truly IID draws from $ G $. Again,... we set the initial prior $ \pi\_{-1} = .5 $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/52.%20Exchangeability%20and%20Bayesian%20Updating_files/52.%20Exchangeability%20and%20Bayesian%20Updating_26_0.png)
</div>
...
We do this by averaging across simulated paths of $ \{\pi*t\}*{t=0}^T $.

Using $ N $ simulated $ \pi*t $ paths, we compute $ 1 - \sum*{i=1}^{N}\pi*{i,t} $ at each $ t $ when the data are generated as draws from $ F $ and compute $ \sum*{i=1}^{N}\pi\_{i,t} $ when the data are generated as draws from $ G $.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/52.%20Exchangeability%20and%20Bayesian%20Updating_files/52.%20Exchangeability%20and%20Bayesian%20Updating_29_0.png)
</div>
...
More insights about the dynamics of $ \{\pi*t\} $ can be gleaned by computing conditional expectations of $ \frac{\pi*{t+1}}{\pi\_{t}} $ as functions of $ \pi_t $ via integration with respect to the pertinent probability distribution:
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/52.%20Exchangeability%20and%20Bayesian%20Updating_files/52.%20Exchangeability%20and%20Bayesian%20Updating_34_0.png)
</div>

The above graphs show that when $ F $ generates the data, $ \pi_t $ on average always heads north, while when $ G $ generates the data, $ \pi_t $ heads south.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/52.%20Exchangeability%20and%20Bayesian%20Updating_files/52.%20Exchangeability%20and%20Bayesian%20Updating_36_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/52.%20Exchangeability%20and%20Bayesian%20Updating_files/52.%20Exchangeability%20and%20Bayesian%20Updating_38_0.png)
</div>
