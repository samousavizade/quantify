...
[[Sutton and Barto, 2018](/courses/Introduction-to-Quantitative-Economics/References#id32)] presents Q-learning and a variety of other statistical learning procedures.
...
Relative to the dynamic programming formulation of the McCall worker model, a Q-learning algorithm gives the worker less knowledge about
...
It differs from the Bellman equation for the McCall model
...
$$
V\left(w\right)=\max_{\text{accept, reject}}\;\left\{ \frac{w}{1-\beta},c+\beta\int V\left(w'\right)dF\left(w'\right)\right\} \tag{33.1}
$$
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_files/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_5_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_files/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_8_0.png)
</div>
...
Note that the first equation of system [(33.2)](#equation-eq-impliedq) presumes that after the agent has accepted an offer, he will not have the objection to reject that same offer in the future.

These equations are aligned with the Bellman equation for the worker’s optimal value function.
...
If we stare at the second equation of system [(33.2)](#equation-eq-impliedq), we notice that since the wage process is identically and independently distributed over time,
...
Instead of using the value function iterations that we deployed above, we can instead iterate to convergence on a version of the second equation in system [(33.2)](#equation-eq-impliedq) that maps an estimate of $ Q_r $ into an improved estimate $ Q_r' $:
...
Thus, construct a difference equation system that keeps the first equation of [(33.3)](#equation-eq-probtosample1)
...
**on average** for a long time series sequence of draws of $ w*t, w*{t+1} $ pairs, where
we are thinking of $ w*t $ as $ w $ and $ w*{t+1} $ as $ w' $.
...
that maps an estimate $ \hat Q*t $ of a Q-function at date $ t $ into an improved estimate
$ \hat Q*{t+1} $ at date
$ t+1 $.
...
$$
\hat Q_{t+1} = \hat Q_t + \alpha \ \textrm{diff}_t \tag{33.6}
$$
...
to objects in equation system [(33.5)](#equation-eq-old105).
...
$$
\widetilde{Q}^{new}\left(w,a\right)=\widetilde{Q}^{old}\left(w,a\right)+\alpha \widetilde{TD}\left(w,a\right) \tag{33.7}
$$
...
in equation [(33.6)](#equation-eq-old106).

An aspect of the algorithm not yet captured by equation system [(33.8)](#equation-eq-old4) is random **experimentation** that
...
1. Update the state associated with the chosen action and compute $ \widetilde{TD} $ according to [(33.8)](#equation-eq-old4) and update $ \widetilde{Q} $ according to [(33.7)](#equation-eq-old3).
1. Either draw a new state $ w' $ if required or else take existing wage if and update the Q-table again according to [(33.7)](#equation-eq-old3).
1. Stop when the old and new Q-tables are close enough, i.e., $ \lVert\tilde{Q}^{new}-\tilde{Q}^{old}\rVert\_{\infty}\leq\delta $ for given $ \delta $ or if the worker keeps accepting for $ T $ periods for a prescribed $ T $.
...
**Remark:** Notice that $ \widetilde{TD} $ associated with an optimal Q-table defined in [(33.7)](#equation-eq-old3) automatically above satisfies $ \widetilde{TD}=0 $ for all state action pairs. Whether a limit of our Q-learning algorithm converges to an optimal Q-table depends on whether the algorithm visits all state-action pairs often enough.
...
```
    ...
    [[5323.8869006  3845.82830933]
     [5306.41663999 5302.27889123]
     [5299.43589718 5215.89553977]
     [5288.2912167  5262.06231014]
     [5237.72103333 5248.49081504]
     [5298.20868137 5295.19112723]
     [5336.76667096 5258.81992048]
     [5316.04291745 5286.08847939]
     [5409.27545723 5262.44328282]
     [5364.10476203 5500.00011291]
     [5475.25625545 6000.        ]]
...
```
    ...
    [5323.8869006  5306.41663999 5299.43589718 5288.2912167  5248.49081504
     5298.20868137 5336.76667096 5316.04291745 5409.27545723 5500.00011291
     6000.        ]
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_files/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_18_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_files/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_20_0.png)
</div>
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_files/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_23_8.png)
</div>
...
The preceding version of temporal difference Q-learning described in equation system [(33.8)](#equation-eq-old4) lets an employed worker quit, i.e., reject her wage as an incumbent and instead receive unemployment compensation this period
and draw a new offer next period.

See [[Ljungqvist and Sargent, 2018](/courses/Introduction-to-Quantitative-Economics/References#id186)], chapter 6 on search, for a proof.
...
It turns out that formulas [(33.9)](#equation-eq-temp-diff) combined with our Q-learning recursion [(33.7)](#equation-eq-old3) can lead our agent to eventually learn the optimal value function as well as in the case where an option to redraw can be exercised.
...
>
  ![png](/static/courses/Advances%20in%20Quantitative%20Economics/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_files/33.%20Job%20Search%20VII%3A%20A%20McCall%20Worker%20Q-Learns_26_10.png)
</div>
