...
To understand the dynamics of the sequence $ (k*t)*{t \geq 0} $ we use a 45-degree diagram.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/solow_files/solow_10_0.png)

Suppose, at some $ k_t $, the value $ g(k_t) $ lies strictly above the 45-degree line.

Then we have $ k\_{t+1} = g(k_t) > k_t $ and capital per worker rises.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/solow_files/solow_12_0.png)

From our graphical analysis, it appears that $ (k_t) $ converges to $ k^\* $, regardless of initial capital
$ k_0 $. This is a form of global stability. The next figure shows three time paths for capital, from
...
![png](/static/courses/Introduction-to-Quantitative-Economics/solow_files/solow_16_0.png)
...
given by $ k\_{t+1} = s f(k_t) + (1 - \delta) k_t $.
...
A **steady state** for [(24.3)](#equation-solowc) is a value $ \hat{k} $
...
The solution is the same as the discrete time case—see [(24.2)](#equation-kstarss).
...
![png](/static/courses/Introduction-to-Quantitative-Economics/solow_files/solow_23_0.png)
...
(You can confirm that this function $ x_t $ satisfies [(24.4)](#equation-xsolow) by
...
Let’s find the value of $ s $ that maximizes $ \hat{c} $ using [scipy.optimize.minimize_scalar](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.minimize_scalar.html#scipy.optimize.minimize_scalar).
We will use $ -\hat{c}(s) $ since `minimize_scalar` finds the minimum value.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/solow_files/solow_34_0.png)

One can also try to solve this mathematically by differentiating $ \hat{c}(s) $ and solve for $ \frac{d}{ds}\hat{c}(s)=0 $ using [sympy](https://www.sympy.org/en/index.html).
...
Let’s define the function _k_next_ to find the next value of $ k $
...
![png](/static/courses/Introduction-to-Quantitative-Economics/solow_files/solow_47_0.png)
