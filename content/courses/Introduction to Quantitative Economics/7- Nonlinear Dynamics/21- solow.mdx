...
A **steady state** for [(24.3)](#equation-solowc) is a value $ k^_ $
...
Let’s find the value of $ s $ that maximizes $ c^_ $ using [scipy.optimize.minimize_scalar](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.minimize_scalar.html#scipy.optimize.minimize_scalar).
We will use $ -c^_(s) $ since `minimize_scalar` finds the minimum value.
...
One can also try to solve this mathematically by differentiating $ c^_(s) $ and solve for $ \frac{d}{ds}c^_(s)=0 $ using [sympy](https://www.sympy.org/en/index.html).
...
k^\alpha_t $ converges to a constant $ y^_ := A (k^_)^\alpha $.
