...
[[Samuelson, 1958](/courses/Introduction-to-Quantitative-Economics/References#id23)].
...
- $ u: \mathbb R\_+ \to \mathbb R $ is called the “flow” utility function
- $ \beta \in (0, 1) $ is the discount factor
- $ c_t $ is time $ t $ consumption of the individual born at time $ t $
- $ c\_{t+1} $ is time $ t+1 $ consumption of the same individual
...
- $ R\_{t+1} $ is the gross interest rate on savings invested at time $ t $, paid at time $ t+1 $

Since $ u $ is strictly increasing, both of these constraints will hold as equalities at the maximum.

Using this fact and substituting $ s*t $ from the first constraint into the second we get
$ c*{t+1} = R\_{t+1}(w_t - c_t) $.

The first-order condition for a maximum can be obtained
by plugging $ c\_{t+1} $ into the objective function, taking the derivative
...
Suppose that, for each $ w*t $ and $ R*{t+1} $, there is exactly one $ s_t $ that
solves [(26.4)](#equation-euler-2-olg).

Then savings can be written as a fixed function of $ w*t $ and $ R*{t+1} $.
...
Together, $ w*t $ and $ R*{t+1} $ represent the _prices_ in the economy (price of
...
The next figure plots the supply of capital, as in [(26.6)](#equation-saving-log-2-olg), as well as the demand for capital, as in [(26.11)](#equation-aggregate-demand-capital-olg), as functions of the interest rate $ R\_{t+1} $.
...
In principle, we can now solve for the equilibrium price $ R\_{t+1} $ given $ w_t $.
...
From equilibrium $ R*{t+1} $ and [(26.11)](#equation-aggregate-demand-capital-olg), we can obtain
the equilibrium quantity $ k*{t+1} $.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/olg_files/olg_19_0.png)
...
The discussion above shows how equilibrium $ k\_{t+1} $ is obtained given $ w_t $.

From [(26.9)](#equation-wage-one) we can translate this into $ k\_{t+1} $ as a function of $ k_t $
...
![png](/static/courses/Introduction-to-Quantitative-Economics/olg_files/olg_23_0.png)
...
We can solve for $ \hat{k} $ by setting $ \hat{k} = g(k^\*) $, or
...
![png](/static/courses/Introduction-to-Quantitative-Economics/olg_files/olg_27_0.png)
...
![png](/static/courses/Introduction-to-Quantitative-Economics/olg_files/olg_29_0.png)
...
![png](/static/courses/Introduction-to-Quantitative-Economics/olg_files/olg_37_0.png)
...
This expression is quite complex and we cannot solve for $ R\_{t+1} $ analytically.
...
Again, with this equation and $ k*t $ as given, we cannot solve for $ k*{t+1} $ by pencil and paper.
...
To solve for $ k\_{t+1} $ given $ k_t $ we use [Newton’s method](https://python.quantecon.org/newton_method.html).
...
If $ k*t $ is given then $ f $ is a function of unknown $ k*{t+1} $.

Then we can use `scipy.optimize.newton` to solve $ f(k*{t+1}, k_t)=0 $ for $ k*{t+1} $.
...
Now let’s define a function that finds the value of $ k\_{t+1} $.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/olg_files/olg_46_0.png)
...
The positive steady state can be obtained by setting $ k\_{t+1} = k_t = k^\* $ in [(26.22)](#equation-law-of-motion-capital-crra), which yields
...
![png](/static/courses/Introduction-to-Quantitative-Economics/olg_files/olg_56_0.png)
