...
![png](/static/courses/Introduction-to-Quantitative-Economics/input_output_files/input_output_6_0.png)
...
After introducing the input-output model, we describe some of its connections
to Linear Programming lecture.
...
- $ z\_{ij} $ be the quantity of good $ i $ allocated to be an input to producing good $ j $ for $ i=1, \ldots n $, $ j =
  1, \ldots n $
- $ z\_{0j} $ be the quantity of labor allocated to producing good $ j $.
- $ a\_{ij} $ be the number of units of good $ i $ required to produce one unit of good $ j $, $ i=0, \ldots, n, j= 1,
  \ldots n $.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/input_output_files/input_output_10_0.png)

_Feasible allocations must satisfy_
...
![png](/static/courses/Introduction-to-Quantitative-Economics/input_output_files/input_output_12_0.png)
...
where $ A $ is the $ n \times n $ matrix with typical element $ a*{ij} $ and $ a_0^\top = \begin{bmatrix} a*{01} &
\cdots & a\_{0n} \end{bmatrix} $.
...
To assure that the solution $ X $ of [(39.2)](#equation-eq-inout-2) is a positive vector, the following **Hawkins-Simon
conditions** suffice:
...
For $ i \in \{1, \ldots , n\} $, the $ i $th component of $ A_0 $ is the amount of labor that is required to produce one
unit of final output of good $ i $.

Equation [(39.4)](#equation-eq-inout-frontier) sweeps out a **production possibility frontier** of final consumption
bundles $ d $ that can be produced with exogenous labor input $ x_0 $.
...
[[Dorfman _et al._, 1958](/courses/Introduction-to-Quantitative-Economics/References#id59)] argue that relative prices of the $ n $
produced goods must satisfy
...
The dual problem chooses prices to maximize the value of a pre-assigned vector of final goods $ d $ subject to prices
...
![png](/static/courses/Introduction-to-Quantitative-Economics/input_output_files/input_output_28_0.png)

## Leontief inverse

We have discussed that gross output $ x $ is given by [(39.2)](#equation-eq-inout-2), where $ L $ is called the Leontief
Inverse.

Recall the Neumann Series Lemma which states that $ L $ exists if the
spectral radius $ r(A)<1 $.
...
This illustrates that an element $ l\_{ij} $ of $ L $ shows the total impact on sector $ i $ of a unit change in demand
of good $ j $.

## Applications of graph theory

We can further study input-output networks through applications
of Graph Theory).
...
In Fig. 39.1 weights are indicated by the widths of the arrows, which are proportional to the corresponding input-output
coefficients.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/input_output_files/input_output_33_0.png)

A higher measure indicates higher importance as a supplier.

As a result demand shocks in most sectors will significantly impact activity in sectors with high eigenvector
centrality.
...
Earlier when disussing demand shocks we concluded that for $ L = (l*{ij}) $ the element
$ l*{ij} $ represents the impact on sector $ i $ of a unit change in demand in sector $ j $.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/input_output_files/input_output_36_0.png)
...
[[Dorfman _et al._, 1958](/courses/Introduction-to-Quantitative-Economics/References#id59)] Chapter 9 discusses an example with the
following
parameter settings:
...
Describe how they infer the input-output coefficients in $ A $ and $ a_0 $ from the following hypothetical underlying
“data” on agricultural and manufacturing industries:
