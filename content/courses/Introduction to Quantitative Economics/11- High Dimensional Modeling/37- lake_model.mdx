---
title: A Lake Model of Employment
draft: false
...
![png](https://intro.quantecon.org/_static/lecture_specific/lake_model/lake_model_worker.png)
...
We can arrange [(40.1)](#equation-lake-model) as a linear system of equations in matrix form $ x\_{t+1} = Ax_t $ where
...
![png](/static/courses/Introduction-to-Quantitative-Economics/lake_model_files/lake_model_7_0.png)
...
Since by intuition if we consider unemployment pool and employment pool as a closed system, the growth should be similar
to the labor force.

We next ask whether the long-run growth rates of $ e_t $ and $ u_t $
also dominated by $ 1+b-d $ as labor force.

The answer will be clearer if we appeal
to Perron-Frobenius theorem
...
begin with an initial condition $ x*0 $ and then evolve recursively by the rule
$ x*{t+1} = Ax_t $ or in short $ x_t = A^tx_0 $.
...
- there exist unique and everywhere positive right eigenvector $ \phi $ (column vector) and left eigenvector $ \psi $ (
  row vector):
...
The last statement implies that the magnitude of $ A^t $ is identical to the magnitude of $ r(A)^t $ in the long run,
where $ r(A) $ can be considered as the dominant eigenvalue in this lecture.
...
$$
\min_j \text{colsum}_j (A) \leq r(A) \leq \max_j \text{colsum}_j (A) \tag{40.2}
$$

Note that $ \text{colsum}\_j(A) = 1 + b - d $ for $ j=1,2 $ and by [(40.2)](#equation-pf-bounds) we can thus conclude
that the dominant eigenvalue
is $ r(A) = 1 + b - d $.

Denote $ g = b - d $ as the overall growth rate of the total labor force, so that $ r(A) = 1 + g $.

The Perron-Frobenius implies that there is a unique positive eigenvector $ \bar{x} = \begin{bmatrix} \bar{u} \\ \bar{e}
\end{bmatrix} $
...
Since $ \bar{x} $ is the eigenvector corresponding to the dominant eigenvalue $ r(A) $, we call $ \bar{x} $ the dominant
eigenvector.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/lake_model_files/lake_model_12_0.png)

Since $ \bar{x} $ is an eigenvector corresponding to the eigenvalue $ r(A) $, all the vectors in the set
$ D := \{ x \in \mathbb{R}^2 : x = \alpha \bar{x} \; \text{for some} \; \alpha >0 \} $ are also eigenvectors
corresponding
to $ r(A) $.

This set $ D $ is represented by a dashed line in the above figure.

The graph illustrates that for two distinct initial conditions $ x*0 $ the sequences of iterates $ (A^t x_0)*{t \geq
0} $ move towards $ D $ over time.

This suggests that all such sequences share strong similarities in the long run, determined by the dominant
eigenvector $ \bar{x} $.
...
What would the behavior of the iterative sequence $ x\_{t+1} = Ax_t $ be now?
...
![png](/static/courses/Introduction-to-Quantitative-Economics/lake_model_files/lake_model_15_0.png)
...
This is a result of the fact that $ r(A)<1 $, which ensures that the iterative sequence $ (A^t x*0)*{t \geq 0} $ will
converge
...
$$
\begin{aligned}
x_t = A^t x_0 &\approx r(A)^t \begin{bmatrix} \bar{u} & \bar{u} \\ \bar{e} & \bar{e} \end{bmatrix} \begin{bmatrix}u_0 \\
e_0 \end{bmatrix} \\
...
We see that the growth of $ u_t $ and $ e_t $ also dominated by $ r(A) = 1+g $ in the long run: $ x_t $ grows along $
D $ as $ r(A) > 1 $ and converges to $ (0, 0) $ as $ r(A) < 1 $.
...
In detail, we have the unemployment rates and employment rates: $ x_t / n_t = A^t n_0 / n_t \to \bar{x} $ as $ t \to
\infty $.
...
One can check that $ \bar{x} $ is also the right eigenvector of $ \hat{A} $ corresponding to $ r(\hat{A}) $ that $
\bar{x} = \hat{A} \bar{x} $.

Moreover, $ \hat{A}^t r_0 \to \bar{x} $ as $ t \to \infty $ for any $ r_0 = x_0 / n_0 $, since the above discussion
implies

$$
r_t = \hat{A}^t r_0 = (1+g)^{-t} A^t r_0 = r(A)^{-t} A^t r_0 \to \begin{bmatrix} \bar{u} & \bar{u} \\ \bar{e} & \bar{e}
\end{bmatrix} r_0 = \begin{bmatrix} \bar{u} \\ \bar{e} \end{bmatrix}.
$$
...
![png](/static/courses/Introduction-to-Quantitative-Economics/lake_model_files/lake_model_18_0.png)

To provide more intuition for convergence, we further explain the convergence below without the Perron-Frobenius
theorem.

Suppose that $ \hat{A} = P D P^{-1} $ is diagonalizable, where $ P = [v_1, v_2] $ consists of eigenvectors $ v_1 $ and $
v_2 $ of $ \hat{A} $
...
The dynamics of the rates follow $ r*{t+1} = \hat{A} r_t $, where $ r_0 $ is a probability vector: $ \sum_j r*{0,j}=1 $.

Consider $ z_t = P^{-1} r_t $.

Then, we have $ z*{t+1} = P^{-1} r*{t+1} = P^{-1} \hat{A} r_t = P^{-1} \hat{A} P z_t = D z_t $.
...
## (Evolution of unemployment and employment rate)Exercise 40.1
...
Plot the graph to illustrate how the line $ D := \{ x \in \mathbb{R}^2 : x = \alpha \bar{x} \; \text{for some} \;
\alpha >0 \} $
shifts in the unemployment-employment space.

## Solution to Exercise 40.1 (Evolution of unemployment and employment rate)

Eq. [(40.3)](#equation-steady-x) implies that the long-run unemployment rate will increase, and the employment rate will
decrease
...
![png](/static/courses/Introduction-to-Quantitative-Economics/lake_model_files/lake_model_23_0.png)
