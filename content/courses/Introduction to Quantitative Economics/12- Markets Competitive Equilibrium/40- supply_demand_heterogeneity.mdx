...
In the previous lectures, we studied competitive equilibria in an economy with many goods.
...
- Relative prices in a competitive equilibrium are the same as those in a special single person or representative consumer economy with preference $ \Pi $ and $ b=\sum*i b_i $, and endowment $ e = \sum_i e*{i} $.
...
Consider a multiple consumer economy with initial distribution of wealth $ W*i $ satisfying $ \sum_i W*{i}=0 $
...
Denote aggregate consumption $ \sum*i c*{i}=c $ and $ \sum_i \mu_i = \mu $.
