---
title: Supply and Demand with Many Goods
draft: false
...
$$
- \frac{1}{2} (\Pi c -b) ^\top (\Pi c -b ) \tag{42.1}
$$
...
The deviation in [(42.3)](#equation-eq-bversusc) will ultimately assure us that competitive equilibrium prices are positive.

### Demand curve implied by constrained utility maximization

For now, we assume that the budget constraint is [(42.2)](#equation-eq-old2).

So we’ll be deriving what is known as a **Marshallian** demand curve.

Our aim is to maximize [(42.1)](#equation-eq-old0) subject to [(42.2)](#equation-eq-old2).
...
$$
c = (\Pi^\top \Pi )^{-1}(\Pi^\top b -  \mu p ) \tag{42.4}
$$

Substituting [(42.4)](#equation-eq-old3) into budget constraint [(42.2)](#equation-eq-old2) and solving for $ \mu $ gives

$$
\mu(p,e) = \frac{p^\top ( \Pi^\top \Pi )^{-1} \Pi^\top b - p^\top e}{p^\top (\Pi^\top \Pi )^{-1} p}. \tag{42.5}
$$

Equation [(42.5)](#equation-eq-old4) tells how marginal utility of wealth depends on the endowment vector $ e $ and the price vector $ p $.

> **Note**Equation [(42.5)](#equation-eq-old4) is a consequence of imposing that $ p^\top (c - e) = 0 $.

We could instead take $ \mu $ as a parameter and use [(42.4)](#equation-eq-old3) and the budget constraint [(42.6)](#equation-eq-old2p) to solve for wealth.
...
In the present case where we have imposed budget constraint in the form [(42.2)](#equation-eq-old2), we are free to normalize the price vector by setting the marginal utility of wealth $ \mu =1 $ (or any other value for that matter).
...
Verify that setting $ \mu=1 $ in [(42.4)](#equation-eq-old3) implies that formula [(42.5)](#equation-eq-old4) is satisfied.

## Exercise 42.2

Verify that setting $ \mu=2 $ in [(42.4)](#equation-eq-old3) also implies that formula
[(42.5)](#equation-eq-old4) is satisfied.
...
Sometimes we’ll use budget constraint [(42.2)](#equation-eq-old2) in situations in which a consumer’s endowment vector $ e $ is his **only** source of income.
...
Whether the consumer’s budget constraint is [(42.2)](#equation-eq-old2) or [(42.6)](#equation-eq-old2p) and whether we take $ w $ as a free parameter or instead as an endogenous variable will affect the consumer’s marginal utility of wealth.

Consequently, how we set $ \mu $ determines whether we are constructing

- a **Marshallian** demand curve, as when we use [(42.2)](#equation-eq-old2) and solve for $ \mu $ using equation [(42.5)](#equation-eq-old4) above, or
- a **Hicksian** demand curve, as when we treat $ \mu $ as a fixed parameter and solve for $ w $ from [(42.6)](#equation-eq-old2p).
...
- **risk** — by interpreting delivery of goods as being contingent on states of the world whose realizations are described by a _known probability distribution_
...
To capture this with our quadratic utility function [(42.1)](#equation-eq-old0), set
...
The budget constraint [(42.2)](#equation-eq-old2) becomes
...
An $ n \times 1 $ vector of marginal revenues for the price-taking firm is

$$
\frac{\partial p^\top q}{\partial q} = p
$$
...
$$
c = ( \Pi^\top \Pi + H )^{-1} ( \Pi^\top b - h ) \tag{42.8}
$$

This equation is the counterpart of Equilibrium Quantity for the scalar $ n=1 $ model with which we began.
...
$$
p = \mu^{-1} (\Pi^\top b - \Pi^\top \Pi c) \tag{42.9}
$$
...
$$
c = (\Pi^\top \Pi + \mu H)^{-1} ( \Pi^\top b - \mu h ) \tag{42.10}
$$
...
[(42.9)](#equation-eq-old5pa).

So the monopolist’s total profits as a function of its output $ q $ is

$$
(\mu^{-1} \Pi^\top (b - \Pi q))^\top  q - h^\top q -  \frac{1}{2} q^\top J q \tag{42.11}
$$
...
$$
q = (H + 2 \mu^{-1} \Pi^\top \Pi)^{-1} (\mu^{-1} \Pi^\top b - h) \tag{42.12}
$$
...
Please verify the monopolist’s supply curve [(42.12)](#equation-eq-qmonop).
...
which implies [(42.10)](#equation-eq-old5p).
