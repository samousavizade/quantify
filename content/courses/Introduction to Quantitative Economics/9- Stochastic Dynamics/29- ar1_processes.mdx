...
of the previous value and an IID shock $ W\_{t+1} $.

(We use $ t+1 $ for the subscript of $ W\_{t+1} $ because this random variable is not
observed at time $ t $.)
...
One of the nice things about this model is that it’s so easy to trace out the sequence of distributions $ \{ \psi_t \} $
...
Given that $ X*t $ is normally distributed, we will know the full distribution
$ \psi_t $ if we can pin down its first two [moments](https://en.wikipedia.org/wiki/Moment*%28mathematics%29).
...
These expressions are obtained from [(32.1)](#equation-can-ar1) by taking, respectively, the expectation and variance of
both sides of the equality.

In calculating the second expression, we are using the fact that $ X*t $
and $ W*{t+1} $ are independent.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_9_0.png)
...
suggesting some kind of stability.
...
![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_11_0.png)
...
![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_13_0.png)
...
(See our lecture on one dimensional dynamics for background on
...
![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_15_0.png)
...
In other words, if $ \psi*t $ is stationary, then $ \psi*{t+j} = \psi_t $ for all $ j $ in $ \mathbb N $.

A different way to put this, specialized to the current setting, is as follows: a density $ \psi $ on $ \mathbb R $ is \*
\*stationary\*\* for the AR(1) process if
...
- The textbook by [[Meyn and Tweedie, 2009](/courses/Introduction-to-Quantitative-Economics/References#id215)] is a classic reference
  on ergodicity.
...
If $ \frac{1}{m} \sum\_{t = 1}^m X_t $ is not close to $ \psi^(x) $, even for many
...
![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_23_0.png)
...
![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_28_0.png)

![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_28_1.png)

![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_28_2.png)
...
First, plot $ \psi*t $ and $ \psi*{t+1} $ using the true
distributions described above.

Second, plot $ \psi\_{t+1} $ on the same figure (in a different
color) as follows:

1. Generate $ n $ draws of $ X_t $ from the $ N(\mu, s^2) $
   distribution
1. Update them all using the rule
   $ X*{t+1} = a X_t + b + c W*{t+1} $
1. Use the resulting sample of $ X\_{t+1} $ values to produce a
   density estimate via kernel density estimation.

Try this for $ n=2000 $ and confirm that the
simulation based estimate of $ \psi\_{t+1} $ does converge to the
...
![png](/static/courses/Introduction-to-Quantitative-Economics/ar1_processes_files/ar1_processes_36_0.png)
