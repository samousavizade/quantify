...
- **Markov Decision Process (MDP):** A mathematical framework for modeling decision-making where outcomes are partly random and partly under the control of a decision-maker. It consists of states, actions, rewards, and transitions.
- **Value Iteration:** An algorithm used to calculate the optimal policy and value function for an MDP by iteratively improving the value function.
- **Q-Learning:** A model-free reinforcement learning algorithm that seeks to find the best action to take given the current state by learning the quality of actions, denoted as Q-values.
- **Deep Q-Networks (DQN):** A reinforcement learning algorithm that combines Q-learning with deep neural networks to handle high-dimensional state spaces.

![png](/static/courses/Introduction-to-Reinforcement-Learning/RL_main.png)]
