...
Let $F*i$ be the information set available at the $i$th transaction. Examples of elements in $F_i$ are $t*{i-j}$, $A*{i-j}$, $D*{i-j}$, and $S\_{i-j}$ for $j \geq 0$. The evolution of price change under model (5.21) can then be partitioned as
